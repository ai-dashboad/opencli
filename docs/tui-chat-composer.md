...
- `opencli-rs/tui/src/bottom_pane/chat_composer.rs`

Paste-burst detector:

- `opencli-rs/tui/src/bottom_pane/paste_burst.rs`
...
- **Persistent history** (cross-session, fetched from `~/.opencli/history.jsonl`): text-only. It
...
(`opencli-rs/tui/src/bottom_pane/chat_composer.rs`). The default config preserves current chat
behavior.
...
`opencli-rs/tui/src/bottom_pane/slash_commands.rs` and reused by both the composer and the command

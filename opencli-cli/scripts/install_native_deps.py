#!/usr/bin/env python3
"""Install OpenCLI native binaries (Rust CLI plus ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/ai-dashboad/opencli/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. opencli-<target>.zst)
...
BINARY_COMPONENTS = {
    "opencli": BinaryComponent(
        artifact_prefix="opencli",
        dest_dir="opencli",
        binary_basename="opencli",
    ),
    "opencli-responses-api-proxy": BinaryComponent(
        artifact_prefix="opencli-responses-api-proxy",
        dest_dir="opencli-responses-api-proxy",
        binary_basename="opencli-responses-api-proxy",
    ),
    "opencli-windows-sandbox-setup": BinaryComponent(
        artifact_prefix="opencli-windows-sandbox-setup",
        dest_dir="opencli",
        binary_basename="opencli-windows-sandbox-setup",
        targets=WINDOWS_TARGETS,
    ),
    "opencli-command-runner": BinaryComponent(
        artifact_prefix="opencli-command-runner",
        dest_dir="opencli",
        binary_basename="opencli-command-runner",
        targets=WINDOWS_TARGETS,
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native OpenCLI binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to opencli, opencli-windows-sandbox-setup,"
            " opencli-command-runner, and rg."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    opencli_cli_root = (args.root or OPENCLI_CLI_ROOT).resolve()
    vendor_dir = opencli_cli_root / VENDOR_DIR_NAME
    vendor_dir.mkdir(parents=True, exist_ok=True)

    components = args.components or [
        "opencli",
        "opencli-windows-sandbox-setup",
        "opencli-command-runner",
        "rg",
    ]

...
OPENCLI_CLI_ROOT = SCRIPT_DIR.parent
REPO_ROOT = OPENCLI_CLI_ROOT.parent
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "opencli-rs" / "responses-api-proxy" / "npm"
OPENCLI_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "opencli": ["opencli", "rg"],
    "opencli-responses-api-proxy": ["opencli-responses-api-proxy"],
    "opencli-sdk": ["opencli"],
}
WINDOWS_ONLY_COMPONENTS: dict[str, list[str]] = {
    "opencli": ["opencli-windows-sandbox-setup", "opencli-command-runner"],
}
COMPONENT_DEST_DIR: dict[str, str] = {
    "opencli": "opencli",
    "opencli-responses-api-proxy": "opencli-responses-api-proxy",
    "opencli-windows-sandbox-setup": "opencli",
    "opencli-command-runner": "opencli",
    "rg": "path",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the OpenCLI CLI npm package.")
    parser.add_argument(
        "--package",
        choices=("opencli", "opencli-responses-api-proxy", "opencli-sdk"),
        default="opencli",
        help="Which npm package to stage (default: opencli).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "opencli":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/opencli.js --version\n"
                    f"    node {staging_dir_str}/bin/opencli.js --help\n\n"
                )
            elif package == "opencli-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/opencli-responses-api-proxy.js --help\n\n"
                )
            else:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="opencli-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    if package == "opencli":
        ...
        shutil.copy2(OPENCLI_CLI_ROOT / "bin" / "opencli.js", bin_dir / "opencli.js")
        rg_manifest = OPENCLI_CLI_ROOT / "bin" / "rg"
        ...
        package_json_path = OPENCLI_CLI_ROOT / "package.json"
    elif package == "opencli-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "opencli-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "opencli-responses-api-proxy.js")
    ...
    elif package == "opencli-sdk":
        package_json_path = OPENCLI_SDK_ROOT / "package.json"
        stage_opencli_sdk_sources(staging_dir)
    else:
...
def stage_opencli_sdk_sources(staging_dir: Path) -> None:
    package_root = OPENCLI_SDK_ROOT
    ...
    if not dist_src.exists():
        raise RuntimeError("opencli-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="opencli-npm-pack-") as pack_dir_str:

...
class WeatherDomain extends TaskDomain {
  ...
  List<DomainIntentPattern> get intentPatterns => [
        DomainIntentPattern(
          pattern: RegExp(
              r'^(?:weather|temperature|temp)(?:\s+(?:in|for|at)\s+(.+?))?(?:\s+tomorrow)?$',
              caseSensitive: false),
        ...
        DomainIntentPattern(
          pattern: RegExp(
              r"^(?:what'?s?\s+the\s+weather)(?:\s+(?:in|for|at)\s+(.+?))?(?:\s+(today|tomorrow))?$",
              caseSensitive: false),
          taskType: 'weather_current',
          extractData: (m) =>
              {'location': m.group(1) ?? '', 'day': m.group(2) ?? 'today'},
        ),
        DomainIntentPattern(
          pattern: RegExp(
              r'^(?:is\s+it\s+going\s+to\s+rain|will\s+it\s+rain)(?:\s+(today|tomorrow))?$',
              caseSensitive: false),
          taskType: 'weather_current',
          extractData: (m) => {'day': m.group(1) ?? 'today'},
        ),
        DomainIntentPattern(
          pattern: RegExp(r'^(?:weather\s+)?forecast(?:\s+(?:for\s+)?(.+))?$',
              caseSensitive: false),
        ...
        DomainOllamaIntent(
          ...
          examples: [
            OllamaExample(
                ...
                intentJson:
                    '{"intent": "weather_current", "confidence": 0.95, "parameters": {}}'),
            OllamaExample(
                input: 'weather in Tokyo',
                intentJson:
                    '{"intent": "weather_current", "confidence": 0.95, "parameters": {"location": "Tokyo"}}'),
            OllamaExample(
                input: 'is it going to rain tomorrow',
                intentJson:
                    '{"intent": "weather_current", "confidence": 0.95, "parameters": {"day": "tomorrow"}}'),
          ],
        ),
        DomainOllamaIntent(
          ...
          examples: [
            OllamaExample(
                input: 'forecast for this week',
                intentJson:
                    '{"intent": "weather_forecast", "confidence": 0.95, "parameters": {}}'),
          ],
        ),
      ];
  ...
  Map<String, DomainDisplayConfig> get displayConfigs => {
        'weather_current': const DomainDisplayConfig(
            cardType: 'weather',
            titleTemplate: 'Weather',
            icon: 'cloud',
            colorHex: 0xFF03A9F4),
        'weather_forecast': const DomainDisplayConfig(
            cardType: 'weather',
            titleTemplate: 'Forecast',
            icon: 'wb_sunny',
            colorHex: 0xFF03A9F4),
      };
  ...
  Future<Map<String, dynamic>> _currentWeather(
    ...
    try {
      final result =
          await Process.run('curl', ['-s', 'wttr.in/$location?format=j1'])
      ...
      return {
        'success': true,
        'location':
            '$cityName, $country'.trim().replaceAll(RegExp(r'^,\s*|,\s*$'), ''),

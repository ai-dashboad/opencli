...
class TranslationDomain extends TaskDomain {
  ...
  String get description =>
      'Translate text between languages using local AI (Ollama)';
  @override
  ...
  List<DomainIntentPattern> get intentPatterns => [
        DomainIntentPattern(
          pattern: RegExp(r'^translate\s+(.+?)\s+(?:to|into)\s+(\w+)$',
              caseSensitive: false),
          taskType: 'translation_translate',
          extractData: (m) => {
            'text': m.group(1)!.trim(),
            'target_language': m.group(2)!.trim()
          },
        ),
        DomainIntentPattern(
          pattern: RegExp(r'^how\s+do\s+you\s+say\s+(.+?)\s+in\s+(\w+)$',
              caseSensitive: false),
          taskType: 'translation_translate',
          extractData: (m) => {
            'text': m.group(1)!.trim(),
        ...
        DomainIntentPattern(
          pattern: RegExp(
              r'^(.+?)\s+in\s+(spanish|french|german|japanese|chinese|korean|italian|portuguese|russian|arabic|hindi)$',
              caseSensitive: false),
          taskType: 'translation_translate',
          extractData: (m) => {
            'text': m.group(1)!.trim(),
        ...
        DomainOllamaIntent(
          ...
          examples: [
            OllamaExample(
                ...
                intentJson:
                    '{"intent": "translation_translate", "confidence": 0.95, "parameters": {"text": "hello", "target_language": "Spanish"}}'),
            OllamaExample(
                input: 'how do you say goodbye in French',
                intentJson:
                    '{"intent": "translation_translate", "confidence": 0.95, "parameters": {"text": "goodbye", "target_language": "French"}}'),
          ],
        ),
      ];
  ...
  Map<String, DomainDisplayConfig> get displayConfigs => {
        'translation_translate': const DomainDisplayConfig(
  ...
  Future<Map<String, dynamic>> _translate(Map<String, dynamic> data) async {
    ...
    try {
      ...
      final prompt =
          'Translate the following text to $targetLang. Return ONLY the translated text, nothing else:\n\n$text';

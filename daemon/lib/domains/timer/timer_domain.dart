...
class TimerDomain extends TaskDomain {
  ...
  String get description =>
      'Set timers, alarms, countdowns, and pomodoro sessions';
  @override
  ...
  List<DomainIntentPattern> get intentPatterns => [
        DomainIntentPattern(
          pattern: RegExp(
              r'^(?:set\s+)?(?:a\s+)?timer\s+(?:for\s+)?(\d+)\s*(min(?:ute)?s?|sec(?:ond)?s?|hour?s?)(?:\s+(.+))?$',
              caseSensitive: false),
          taskType: 'timer_set',
          extractData: (m) {
        ...
        DomainIntentPattern(
          pattern: RegExp(r'^(\d+)\s*(?:min(?:ute)?s?)\s+timer$',
              caseSensitive: false),
          taskType: 'timer_set',
          extractData: (m) =>
              {'minutes': int.parse(m.group(1)!), 'label': 'Timer'},
        ),
        DomainIntentPattern(
        ...
        DomainIntentPattern(
          pattern:
              RegExp(r'^(?:focus\s+timer)\s*(\d+)?$', caseSensitive: false),
          taskType: 'timer_pomodoro',
        ...
        DomainIntentPattern(
          pattern: RegExp(r'^(?:timer\s+status|how\s+much\s+time\s+left)$',
              caseSensitive: false),
  ...
  List<DomainOllamaIntent> get ollamaIntents => [
        DomainOllamaIntent(
          ...
          examples: [
            OllamaExample(
                input: 'set timer for 25 minutes',
                intentJson:
                    '{"intent": "timer_set", "confidence": 0.95, "parameters": {"minutes": 25, "label": "Timer"}}'),
            OllamaExample(
                input: 'timer 10 min cooking',
                intentJson:
                    '{"intent": "timer_set", "confidence": 0.95, "parameters": {"minutes": 10, "label": "cooking"}}'),
          ],
        ),
        DomainOllamaIntent(
          intentName: 'timer_pomodoro',
          description:
              'Start a pomodoro focus timer (25 min work + 5 min break)',
          parameters: {},
          examples: [
            OllamaExample(
                input: 'start pomodoro',
                intentJson:
                    '{"intent": "timer_pomodoro", "confidence": 0.95, "parameters": {}}'),
          ],
        ),
      ];
  ...
  Map<String, DomainDisplayConfig> get displayConfigs => {
        'timer_set': const DomainDisplayConfig(
          cardType: 'timer',
          titleTemplate: 'Timer: \${label}',
          subtitleTemplate: '\${minutes} minutes',
          icon: 'timer',
          colorHex: 0xFF009688,
        ),
        'timer_status': const DomainDisplayConfig(
          cardType: 'timer',
          titleTemplate: 'Timer Status',
          icon: 'timer',
          colorHex: 0xFF009688,
        ),
        'timer_pomodoro': const DomainDisplayConfig(
          cardType: 'timer',
          titleTemplate: 'Pomodoro',
          subtitleTemplate: '25 min focus',

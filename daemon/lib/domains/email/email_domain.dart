...
class EmailDomain extends TaskDomain {
  ...
  List<DomainOllamaIntent> get ollamaIntents => [
    DomainOllamaIntent(
      ...
      examples: [
        OllamaExample(input: 'email john@example.com about the meeting', intentJson: '{"intent": "email_compose", "confidence": 0.95, "parameters": {"to": "john@example.com", "subject": "the meeting"}}'),
        OllamaExample(input: 'send email to the team about standup', intentJson: '{"intent": "email_compose", "confidence": 0.95, "parameters": {"to": "the team", "subject": "standup"}}'),
      ],
    ),
    DomainOllamaIntent(
      ...
      examples: [
        OllamaExample(input: 'check my email', intentJson: '{"intent": "email_check", "confidence": 0.95, "parameters": {}}'),
        OllamaExample(input: 'any new mail', intentJson: '{"intent": "email_check", "confidence": 0.95, "parameters": {}}'),
      ],
    ),
  ];
  ...
  Map<String, DomainDisplayConfig> get displayConfigs => {
    'email_compose': const DomainDisplayConfig(cardType: 'email', titleTemplate: 'Email Composed', icon: 'send', colorHex: 0xFFF44336),
    'email_check': const DomainDisplayConfig(cardType: 'email', titleTemplate: 'Inbox', icon: 'inbox', colorHex: 0xFFF44336),
  };
  ...
  activate
    ...
    try {
      ...
      return {
        ...
        'to': to,
        'subject': subject,
        'message': result.exitCode == 0 ? 'Email draft opened for $to' : (result.stderr as String).trim(),
  ...
  return unreadCount as string
    ...
    try {
      ...
      return {
        'success': result.exitCode == 0,
        'unread_count': count,
        'message': count > 0 ? 'You have $count unread email(s)' : 'No unread emails',
        'domain': 'email', 'card_type': 'email',
      };
    } catch (e) {
      return {'success': false, 'error': 'Error: $e', 'domain': 'email'};

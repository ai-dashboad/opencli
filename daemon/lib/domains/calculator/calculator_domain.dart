...
class CalculatorDomain extends TaskDomain {
  ...
  @override
  String get description => 'Math calculations, unit conversions, timezone, and date math';
  @override
  ...
  List<String> get taskTypes => [
    'calculator_eval', 'calculator_convert', 'calculator_timezone', 'calculator_date_math',
  ];
  ...
  List<DomainOllamaIntent> get ollamaIntents => [
    DomainOllamaIntent(
      ...
      examples: [
        OllamaExample(input: 'what is 15% of 234', intentJson: '{"intent": "calculator_eval", "confidence": 0.95, "parameters": {"expression": "15% of 234"}}'),
        OllamaExample(input: 'calculate sqrt of 144', intentJson: '{"intent": "calculator_eval", "confidence": 0.95, "parameters": {"expression": "sqrt(144)"}}'),
      ],
    ),
    DomainOllamaIntent(
      intentName: 'calculator_convert',
      description: 'Convert between units (distance, temperature, weight)',
      parameters: {'value': 'number', 'from': 'source unit', 'to': 'target unit'},
      examples: [
        OllamaExample(input: '5 miles to km', intentJson: '{"intent": "calculator_convert", "confidence": 0.95, "parameters": {"value": 5, "from": "miles", "to": "km"}}'),
        OllamaExample(input: '100 fahrenheit to celsius', intentJson: '{"intent": "calculator_convert", "confidence": 0.95, "parameters": {"value": 100, "from": "fahrenheit", "to": "celsius"}}'),
      ],
    ),
    DomainOllamaIntent(
      ...
      examples: [
        OllamaExample(input: 'what time is it in Tokyo', intentJson: '{"intent": "calculator_timezone", "confidence": 0.95, "parameters": {"location": "Tokyo"}}'),
      ],
    ),
  ];
  ...
  Map<String, DomainDisplayConfig> get displayConfigs => {
    'calculator_eval': const DomainDisplayConfig(
      cardType: 'calculator', titleTemplate: 'Calculator',
      icon: 'calculate', colorHex: 0xFF3F51B5,
    ),
    'calculator_convert': const DomainDisplayConfig(
      cardType: 'calculator', titleTemplate: 'Conversion',
      icon: 'swap_horiz', colorHex: 0xFF3F51B5,
    ),
    'calculator_timezone': const DomainDisplayConfig(
      cardType: 'calculator', titleTemplate: 'Timezone',
      icon: 'public', colorHex: 0xFF3F51B5,
    ),
    'calculator_date_math': const DomainDisplayConfig(
      cardType: 'calculator', titleTemplate: 'Date Calculation',
      icon: 'date_range', colorHex: 0xFF3F51B5,
    ),
  };
  ...
  Future<Map<String, dynamic>> executeTask(String taskType, Map<String, dynamic> data) async {
    switch (taskType) {
      ...
      default:
        return {'success': false, 'error': 'Unknown calculator task: $taskType'};
    }
  }
  ...
  Map<String, dynamic> _evaluate(Map<String, dynamic> data) {
    ...
    try {
      ...
      final percentMatch = RegExp(r'(\d+(?:\.\d+)?)\s*%\s*(?:of)\s+(\d+(?:\.\d+)?)').firstMatch(expr);
      if (percentMatch != null) {
        final pct = double.parse(percentMatch.group(1)!);
        final value = double.parse(percentMatch.group(2)!);
        final result = (pct / 100) * value;
        return {'success': true, 'expression': expr, 'result': _formatNumber(result), 'domain': 'calculator', 'card_type': 'calculator'};
      }

      // Handle sqrt
      final sqrtMatch = RegExp(r'sqrt\s*\(?(\d+(?:\.\d+)?)\)?').firstMatch(expr);
      if (sqrtMatch != null) {
        final val = double.parse(sqrtMatch.group(1)!);
        return {'success': true, 'expression': expr, 'result': _formatNumber(math.sqrt(val)), 'domain': 'calculator', 'card_type': 'calculator'};
      }

      // Handle power: "2^10" or "2 power 10"
      final powMatch = RegExp(r'(\d+(?:\.\d+)?)\s*[\^]\s*(\d+(?:\.\d+)?)').firstMatch(expr);
      if (powMatch != null) {
        final base = double.parse(powMatch.group(1)!);
        final exp = double.parse(powMatch.group(2)!);
        return {'success': true, 'expression': expr, 'result': _formatNumber(math.pow(base, exp).toDouble()), 'domain': 'calculator', 'card_type': 'calculator'};
      }
      ...
      return {'success': false, 'expression': expr, 'error': 'Could not evaluate expression', 'domain': 'calculator'};
    } catch (e) {
      return {'success': false, 'expression': expr, 'error': 'Calculation error: $e', 'domain': 'calculator'};
    }
  }
  ...
  Map<String, dynamic> _convert(Map<String, dynamic> data) {
    ...
    if (_isTemp(from) && _isTemp(to)) {
      ...
      if (result != null) {
        return {
          'success': true, 'value': value, 'from': from, 'to': to,
          'result': _formatNumber(result),
          'display': '${_formatNumber(value)} $from = ${_formatNumber(result)} $to',
    ...
    return {'success': false, 'error': 'Unknown conversion: $from to $to', 'domain': 'calculator'};
  }
  ...
  double? _convertTemp(double value, String from, String to) {
    final f = from.startsWith('f') ? 'f' : from.startsWith('c') ? 'c' : 'k';
    final t = to.startsWith('f') ? 'f' : to.startsWith('c') ? 'c' : 'k';
    if (f == t) return value;
  ...
  Map<String, dynamic> _timezone(Map<String, dynamic> data) {
    ...
    final tzOffsets = <String, int>{
      ...
      'beijing': 8, 'china': 8, 'shanghai': 8, 'cst_china': 8,
    ...
    if (offset == null) {
      return {'success': false, 'error': 'Unknown timezone/city: $location', 'domain': 'calculator'};
    }

    final utcNow = DateTime.now().toUtc();
    final localTime = utcNow.add(Duration(hours: offset));
    final formatted = '${localTime.hour.toString().padLeft(2, '0')}:${localTime.minute.toString().padLeft(2, '0')}';
    ...
    return {
      ...
      'display': 'It\'s $formatted in ${location[0].toUpperCase()}${location.substring(1)} ($dateStr, UTC${offset >= 0 ? '+' : ''}$offset)',
      'domain': 'calculator', 'card_type': 'calculator',
    };
  }
  ...
  Map<String, dynamic> _dateMath(Map<String, dynamic> data) {
    ...
    if (operation == 'days_from_now') {
      ...
      return {
        'success': true, 'days': days, 'date': dateStr,
    ...
    if (operation == 'days_until') {
      ...
      if (targetDate == null) {
        return {'success': false, 'error': 'Could not parse date: $target', 'domain': 'calculator'};
      }
      final days = targetDate.difference(now).inDays;
      return {
        'success': true, 'target': target, 'days': days,
  ...
  DateTime? _parseDate(String text) {
    ...
    final months = {'january': 1, 'february': 2, 'march': 3, 'april': 4, 'may': 5, 'june': 6,
    ...
    for (final entry in months.entries) {
      ...
      if (match != null) {
        ...
        if (d.isBefore(now)) d = DateTime(year + 1, entry.value, int.parse(match.group(1)!));
        return d;
      }
    }

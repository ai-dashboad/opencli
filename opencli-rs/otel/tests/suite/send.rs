...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    ...
    let counter = find_metric(&resource_metrics, "opencli.turns").expect("counter metric missing");
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "opencli.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "opencli.tool_latency").and_then(|metric| {
            match metric.data() {
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    metrics.counter("opencli.alpha", 1, &[("env", "dev"), ("component", "alpha")])?;
    metrics.counter(
        "opencli.beta",
        2,
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "opencli.alpha").expect("opencli.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "opencli.beta").expect("opencli.beta metric missing");
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("opencli.turns", 1, &[])?;
    metrics.shutdown()?;

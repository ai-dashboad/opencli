...
use opencli_app_server_protocol::TextPosition as AppTextPosition;
use opencli_app_server_protocol::TextRange as AppTextRange;
use opencli_core::ExecPolicyError;
use opencli_core::check_execpolicy_for_warnings;
use opencli_core::config_loader::ConfigLoadError;
use opencli_core::config_loader::TextRange as CoreTextRange;
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
        ...
        .get_layers(ConfigLayerStackOrdering::LowestPrecedenceFirst, true)
    {
        ...
        if let ConfigLayerSource::Project { dot_opencli_folder } = &layer.name {
...
pub async fn run_main(
    ...
    let feedback = OpenCLIFeedback::new();

    let otel = opencli_core::otel_init::build_provider(
        &config,

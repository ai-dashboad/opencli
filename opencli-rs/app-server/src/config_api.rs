...
use opencli_app_server_protocol::ConfigRequirements;
use opencli_app_server_protocol::ConfigRequirementsReadResponse;
use opencli_app_server_protocol::ConfigValueWriteParams;
use opencli_app_server_protocol::ConfigWriteErrorCode;
...
use opencli_core::config::ConfigServiceError;
use opencli_core::config_loader::CloudRequirementsLoader;
use opencli_core::config_loader::ConfigRequirementsToml;
use opencli_core::config_loader::LoaderOverrides;
use opencli_core::config_loader::ResidencyRequirement as CoreResidencyRequirement;
use opencli_core::config_loader::SandboxModeRequirement as CoreSandboxModeRequirement;
use serde_json::json;
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(opencli_app_server_protocol::AskForApproval::from)
                .collect()
        }),
...
) -> opencli_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => opencli_app_server_protocol::ResidencyRequirement::Us,
    }
}
...
mod tests {
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        ...
        assert_eq!(
            ...
            Some(vec![
                opencli_app_server_protocol::AskForApproval::Never,
                opencli_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(opencli_app_server_protocol::ResidencyRequirement::Us),
        );
    }
}

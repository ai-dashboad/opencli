...
use opencli_app_server_protocol::CancelLoginChatGptParams;
use opencli_app_server_protocol::ClientInfo;
use opencli_app_server_protocol::ClientNotification;
...
pub const DEFAULT_CLIENT_NAME: &str = "opencli-app-server-tests";

impl McpProcess {
    pub async fn new(opencli_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(opencli_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = opencli_utils_cargo_bin::cargo_bin("opencli-app-server")
            .context("should find binary for opencli-app-server")?;
        ...
        cmd.env_remove(OPENCLI_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("opencli-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()

...
const OPENCLI_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are OpenCLI, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let opencli_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(opencli_home.path(), &server.uri())?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(OPENCLI_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn create_config_toml(opencli_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
...
fn setup_rollout_fixture(opencli_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(opencli_home, server_uri)?;
    ...
    let rollout_file_path = rollout_path(opencli_home, filename_ts, &conversation_id);

...
async fn list_apps_returns_connectors_with_accessible_flags() -> Result<()> {
    ...
    let opencli_home = TempDir::new()?;
    write_connectors_config(opencli_home.path(), &server_url)?;
    write_chatgpt_auth(
        opencli_home.path(),
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/opencli/apps", mcp_service);
...
fn write_connectors_config(opencli_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {

...
fn create_config_toml(opencli_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(opencli_home.path(), CreateConfigTomlParams::default())?;
...
async fn set_auth_token_updates_account_and_notifies() -> Result<()> {
    ...
    write_models_cache(opencli_home.path())?;
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(opencli_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<opencli_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(opencli_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![opencli_app_server_protocol::UserInput::Text {

...
use opencli_app_server_protocol::PatchChangeKind;
use opencli_app_server_protocol::RequestId;
use opencli_app_server_protocol::ServerRequest;
use opencli_app_server_protocol::TextElement;
...
use opencli_core::protocol_config_types::ReasoningSummary;
...
const TEST_ORIGINATOR: &str = "opencli_vscode";
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("OpenCLI VS Code Extension".to_string()),
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three OpenCLI turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
    ...
    assert_eq!(
        started.turn.status,
        opencli_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    // Two OpenCLI turns hit the mock model (session start + turn/start).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    ...
    let image_path = opencli_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(opencli_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": opencli_core::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("opencli/event/task_complete"),
    )
    .await??;
    timeout(
        DEFAULT_READ_TIMEOUT,
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(opencli_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(opencli_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let opencli_home = tmp.path().join("opencli_home");
    std::fs::create_dir(&opencli_home)?;
    ...
    let mut mcp = McpProcess::new(&opencli_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(opencli_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(opencli_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![opencli_app_server_protocol::FileUpdateChange {

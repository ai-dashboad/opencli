...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    create_config_toml(opencli_home.path(), &server.uri(), "never")?;
    let mut mcp = McpProcess::new(opencli_home.path()).await?;

    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "opencli_vscode".to_string(),
    ...
    assert!(user_agent.starts_with("opencli_vscode/"));
    Ok(())
}
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        ...
        &[(
            ...
            Some("opencli_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("opencli_originator_via_env_var/"));
    Ok(())
}
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        opencli_home.path(),
        &[("OPENCLI_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;

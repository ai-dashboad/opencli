...
async fn get_user_agent_returns_current_opencli_user_agent() -> Result<()> {
    ...
    let terminal_ua = opencli_core::terminal::user_agent();
    // Read the version rather than hard-coding it: pinning `0.0.0` here means
    // this test fails on every release.
    let version = env!("CARGO_PKG_VERSION");
    let user_agent = format!(
        "{originator}/{version} ({os_type} {os_version}; {architecture}) {terminal_ua} ({DEFAULT_CLIENT_NAME}; 0.1.0)"
    );

...
- The legacy rule matcher lives in `opencli-execpolicy-legacy`.
...
- From the OpenCLI CLI, run `opencli execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
opencli execpolicy check --rules path/to/policy.rules git status
```
...
```bash
cargo run -p opencli-execpolicy -- check --rules path/to/policy.rules git status
```

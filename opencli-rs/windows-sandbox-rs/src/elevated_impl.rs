mod windows_impl {
    ...
    /// Creates the sandbox user's OpenCLI home directory if it does not already exist.
    fn ensure_opencli_home_exists(p: &Path) -> Result<()> {
    ...
    /// Locates `opencli-command-runner.exe` next to the current binary.
    fn find_runner_exe() -> PathBuf {
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                let candidate = dir.join("opencli-command-runner.exe");
                if candidate.exists() {
                    return candidate;
                }
            }
        }
        PathBuf::from("opencli-command-runner.exe")
    }
    ...
    struct RunnerPayload {
        ...
        // Writable log dir for sandbox user (.opencli in sandbox profile).
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = opencli_home.join(".sandbox");
        ensure_opencli_home_exists(&sandbox_base)?;

        let logs_base_dir: Option<&Path> = Some(sandbox_base.as_path());
        log_start(&command, logs_base_dir);
        let sandbox_creds =
            require_logon_sandbox_creds(&policy, sandbox_policy_cwd, cwd, &env_map, opencli_home)?;
        ...
        let caps = load_or_create_cap_sids(opencli_home)?;
        ...
        let runner_cmdline = runner_exe
            .to_str()
            .map(|s| s.to_string())
            .unwrap_or_else(|| "opencli-command-runner.exe".to_string());
        ...
        let payload = RunnerPayload {
            ...
            opencli_home: sandbox_base.clone(),
            real_opencli_home: opencli_home.to_path_buf(),

...
use base64::Engine;
use opencli_windows_sandbox::convert_string_sid_to_sid;
use opencli_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use opencli_windows_sandbox::ensure_allow_write_aces;
use opencli_windows_sandbox::extract_setup_failure;
use opencli_windows_sandbox::hide_newly_created_users;
use opencli_windows_sandbox::load_or_create_cap_sids;
use opencli_windows_sandbox::log_note;
use opencli_windows_sandbox::path_mask_allows;
use opencli_windows_sandbox::sandbox_dir;
use opencli_windows_sandbox::sandbox_secrets_dir;
use opencli_windows_sandbox::string_from_sid_bytes;
use opencli_windows_sandbox::to_wide;
use opencli_windows_sandbox::write_setup_error_report;
use opencli_windows_sandbox::SetupErrorCode;
use opencli_windows_sandbox::SetupErrorReport;
use opencli_windows_sandbox::SetupFailure;
use opencli_windows_sandbox::LOG_FILE_NAME;
use opencli_windows_sandbox::SETUP_VERSION;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(opencli_home) = std::env::var("OPENCLI_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&opencli_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.opencli_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.opencli_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.opencli_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.opencli_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.opencli_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.opencli_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.opencli_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.opencli_home).join("sandbox_users.json");

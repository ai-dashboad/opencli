...
pub const OFFLINE_USERNAME: &str = "OpenCLISandboxOffline";
pub const ONLINE_USERNAME: &str = "OpenCLISandboxOnline";
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;

pub fn sandbox_dir(opencli_home: &Path) -> PathBuf {
    opencli_home.join(".sandbox")
}

pub fn sandbox_secrets_dir(opencli_home: &Path) -> PathBuf {
    opencli_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(opencli_home: &Path) -> PathBuf {
    sandbox_dir(opencli_home).join("setup_marker.json")
}

pub fn sandbox_users_path(opencli_home: &Path) -> PathBuf {
    sandbox_secrets_dir(opencli_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| opencli_home.to_path_buf());
    log_note(
        &format!(
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("opencli-windows-sandbox-setup.exe");
            if candidate.exists() {
                return candidate;
            }
        }
    }
    PathBuf::from("opencli-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(opencli_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(opencli_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(opencli_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(opencli_home);
    ...
    run_setup_exe(&payload, needs_elevation, opencli_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, opencli_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, opencli_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to OPENCLI_HOME or anything under OPENCLI_HOME/.sandbox.
    // These locations contain sandbox control/state and must remain tamper-resistant.
    let opencli_home_key = crate::audit::normalize_path_key(opencli_home);
    let sbx_dir_key = crate::audit::normalize_path_key(&sandbox_dir(opencli_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let secrets_dir_key = crate::audit::normalize_path_key(&sandbox_secrets_dir(opencli_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = crate::audit::normalize_path_key(root);
        key != opencli_home_key
            && key != sbx_dir_key

...
pub fn cap_sid_file(opencli_home: &Path) -> PathBuf {
    opencli_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(opencli_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(opencli_home);
    if path.exists() {

...
# Run a suite of smoke tests against the Windows sandbox via the OpenCLI CLI
...
def _resolve_opencli_cmd() -> List[str]:
    """Resolve the OpenCLI CLI to invoke `opencli sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run OpenCLI.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "opencli.exe",
        ws_root / "target" / "release" / "opencli.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "opencli.exe",
            cargo_base / "release" / "opencli.exe",
        ])
    ...
    raise FileNotFoundError(
        "OpenCLI CLI not found. Build it first, e.g.\n"
        "  cargo build -p opencli-cli --release\n"
        "or for debug:\n"
        "  cargo build -p opencli-cli\n"
    )

OPENCLI_CMD = _resolve_opencli_cmd()
print(OPENCLI_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to opencli CLI flags
    ...
    argv = [*OPENCLI_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\opencli_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.opencli artifacts) denied
    opencli_home = Path(os.environ["USERPROFILE"]) / ".opencli"
    cap_sid_target = opencli_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .opencli\\policy.json"], WS_ROOT)
    add("WS: .opencli cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .opencli policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > opencli_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://opencli-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(

...
use opencli_core::config_loader::load_config_layers_state;
...
pub(crate) async fn build_config_state() -> Result<ConfigState> {
    // Load config through `opencli-core` so we inherit the same layer ordering and semantics as the
    // rest of OpenCLI (system/managed layers, user layers, session flags, etc.).
    ...
    .await
    .context("failed to load OpenCLI config")?;

    let cfg_path = opencli_home.join(CONFIG_TOML_FILE);
    ...
    stack
        ...
        .filter_map(|layer| {
            let path = match &layer.name {
                ...
                ConfigLayerSource::Project { dot_opencli_folder } => dot_opencli_folder
...
fn enforce_trusted_constraints(
    layers: &opencli_core::config_loader::ConfigLayerStack,
...
fn network_proxy_constraints_from_trusted_layers(
    ...
    for layer in layers.get_layers(
        opencli_core::config_loader::ConfigLayerStackOrdering::LowestPrecedenceFirst,
        false,
    ) {

...
pub fn blocked_message(reason: &str) -> &'static str {
    match reason {
        REASON_NOT_ALLOWED => "OpenCLI blocked this request: domain not in allowlist.",
        REASON_NOT_ALLOWED_LOCAL => {
            "OpenCLI blocked this request: local/private addresses not allowed."
        }
        REASON_DENIED => "OpenCLI blocked this request: domain denied by policy.",
        REASON_METHOD_NOT_ALLOWED => {
            "OpenCLI blocked this request: method not allowed in limited mode."
        }
        _ => "OpenCLI blocked this request by network policy.",
    }
}

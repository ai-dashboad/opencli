# opencli-network-proxy

`opencli-network-proxy` is OpenCLI's local network policy enforcement proxy. It runs:
...
`opencli-network-proxy` reads from OpenCLI's merged `config.toml` (via `opencli-core` config loading).
...
```bash
cargo run -p opencli-network-proxy --
```
...
`opencli-network-proxy` can be embedded as a library with a thin API:

```rust
use opencli_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
This section documents the protections implemented by `opencli-network-proxy`, and the boundaries of

...
where
    P: AsRef<Path>,
{
    let mcp_executable = opencli_utils_cargo_bin::cargo_bin("opencli-exec-mcp-server")?;
    let execve_wrapper = opencli_utils_cargo_bin::cargo_bin("opencli-execve-wrapper")?;
    ...
    let transport = TokioChildProcess::new(Command::new(&mcp_executable).configure(|cmd| {
        ...
        cmd.env("OPENCLI_HOME", opencli_home.as_ref());
...
pub async fn write_default_execpolicy<P>(policy: &str, opencli_home: P) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
    let policy_dir = opencli_home.as_ref().join("rules");

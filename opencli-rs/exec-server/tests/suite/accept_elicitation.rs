...
async fn accept_elicitation_for_prompt_rule() -> Result<()> {
    ...
    // $OPENCLI_HOME with an execpolicy that prompts for `git init` commands.
...
"#,
    ...
    let transport = create_transport(opencli_home.as_ref(), dotslash_cache).await?;
    ...
    let response =
        notify_readable_sandbox(&project_root_path, opencli_linux_sandbox_exe, &service).await?;
...
fn ensure_opencli_cli() -> Result<PathBuf> {
    let opencli_cli = opencli_utils_cargo_bin::cargo_bin("opencli")?;

    let metadata = opencli_cli.metadata().with_context(|| {
        format!(
            "failed to read metadata for opencli binary at {}",
            opencli_cli.display()
        )
    })?;
    ensure!(
        metadata.is_file(),
        "expected opencli binary at {} to be a file; run `cargo build -p opencli-cli --bin opencli` before this test",
        opencli_cli.display()
    );
    ...
    let mode = metadata.permissions().mode();
        ...
        mode & 0o111 != 0,
        "opencli binary at {} is not executable (mode {mode:o}); run `cargo build -p opencli-cli --bin opencli` before this test",
        opencli_cli.display()
    );

...
use anyhow::Result;
use opencli_core::MCP_SANDBOX_STATE_CAPABILITY;
use opencli_core::MCP_SANDBOX_STATE_METHOD;
use opencli_core::SandboxState;
...
const OPENCLI_BASH_PATH_ENV_VAR: &str = "OPENCLI_BASH_PATH";

const SANDBOX_STATE_CAPABILITY_VERSION: &str = "1.0.0";

pub(crate) fn get_bash_path() -> Result<PathBuf> {
    std::env::var(OPENCLI_BASH_PATH_ENV_VAR)
        .map(PathBuf::from)
        .context(format!("{OPENCLI_BASH_PATH_ENV_VAR} must be set"))
}
...
impl ExecTool {
    ...
    async fn shell(
        ...
        let effective_timeout = Duration::from_millis(
            params
                .timeout_ms
                .unwrap_or(opencli_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS),
        );
...
#[derive(Default)]
pub struct OpenCLISandboxStateUpdateMethod;

impl rmcp::model::ConstString for OpenCLISandboxStateUpdateMethod {
...
impl ServerHandler for ExecTool {
    ...
    async fn on_custom_request(
        ...
        if method != MCP_SANDBOX_STATE_METHOD {
            return Err(McpError::method_not_found::<OpenCLISandboxStateUpdateMethod>());
        }

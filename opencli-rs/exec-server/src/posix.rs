...
//! from us, and we give its fd number in the OPENCLI_ESCALATE_SOCKET environment variable.
//!
...
use opencli_core::is_dangerous_command::command_might_be_dangerous;
...
const OPENCLI_EXECVE_WRAPPER_EXE_NAME: &str = "opencli-execve-wrapper";
...
pub async fn main_mcp_server() -> anyhow::Result<()> {
    ...
    let execve_wrapper = match cli.execve_wrapper {
        ...
        None => {
            ...
            cwd.parent()
                .map(|p| p.join(OPENCLI_EXECVE_WRAPPER_EXE_NAME))
                .ok_or_else(|| {
...
async fn load_exec_policy() -> anyhow::Result<Policy> {
    ...
    let overrides = opencli_core::config_loader::LoaderOverrides::default();
    let config_layer_stack = opencli_core::config_loader::load_config_layers_state(
        &opencli_home,
        cwd,
        &cli_overrides,
        overrides,
        opencli_core::config_loader::CloudRequirementsLoader::default(),
    )
    .await?;

    opencli_core::load_exec_policy(&config_layer_stack)
        .await

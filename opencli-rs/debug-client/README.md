WARNING: this code is mainly generated by OpenCLI and should not be used in production

# opencli-debug-client

A tiny interactive client for `opencli app-server` (protocol v2 only). It prints
all JSON-RPC lines from the server and lets you send new turns as you type.

## Usage

Start the app-server client (it will spawn `opencli app-server` itself):

```
cargo run -p opencli-debug-client -- \
  --opencli-bin opencli \
...
```
cargo run -p opencli-debug-client -- --thread-id thr_123
```

### CLI flags

- `--opencli-bin <path>`: path to the `opencli` binary (default: `opencli`).
- `-c, --config key=value`: pass through `--config` overrides to `opencli`.

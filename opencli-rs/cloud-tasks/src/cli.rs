...
pub enum Command {
    /// Submit a new OpenCLI Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a OpenCLI Cloud task.
    Status(StatusCommand),
    /// List OpenCLI Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a OpenCLI Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a OpenCLI Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in OpenCLI Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `opencli cloud` to browse).
    ...
    /// Git branch to run in OpenCLI Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// OpenCLI Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// OpenCLI Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// OpenCLI Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,

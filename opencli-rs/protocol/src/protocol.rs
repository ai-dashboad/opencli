//! Defines the protocol for a OpenCLI session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for OpenCLI:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::opencli_thread::OpenCLIThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by opencli-core; it does not
    /// involve the model.
    SetThreadName { name: String },

    /// Request OpenCLI to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request OpenCLI to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down opencli instance.
    Shutdown,
...
/// running the command proposed by OpenCLI.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.opencli`, `.git`, notably `.git/hooks`) under
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                roots
                    ...
                    .map(|writable_root| {
                        ...
                        #[allow(clippy::expect_used)]
                        let top_level_opencli = writable_root
                            .join(".opencli")
                            .expect(".opencli is a valid relative path");
                        if top_level_opencli.as_path().is_dir() {
                            subpaths.push(top_level_opencli);
                        }
                        WritableRoot {
                            root: writable_root,
...
/// OpenCLI errors that we expose to clients.

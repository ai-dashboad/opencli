...
const METRIC_DB_INIT: &str = "opencli.db.init";
...
impl StateRuntime {
    /// Initialize the state runtime using the provided OpenCLI home and default provider.
    ///
    /// This opens (and migrates) the SQLite database at `opencli_home/state.sqlite`.
    pub async fn init(
        opencli_home: PathBuf,
        default_provider: String,
        ...
        let state_path = opencli_home.join(STATE_DB_FILENAME);
    ...
    /// Return the configured OpenCLI home directory for this runtime.
    pub fn opencli_home(&self) -> &Path {
        self.opencli_home.as_path()
    }

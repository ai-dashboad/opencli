...
#[derive(Debug, Parser)]
#[command(name = "opencli-state-logs")]
#[command(about = "Tail OpenCLI logs from state.sqlite with simple filters")]
struct Args {
    /// Path to OPENCLI_HOME. Defaults to $OPENCLI_HOME or ~/.opencli.
    #[arg(long, env = "OPENCLI_HOME")]
    opencli_home: Option<PathBuf>,

    /// Direct path to the SQLite database. Overrides --opencli-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let opencli_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(opencli_home, "logs-client".to_string(), None).await?;
...
fn default_opencli_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".opencli");
    }
    PathBuf::from(".opencli")
}

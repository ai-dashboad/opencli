...
use opencli_api::provider::RetryConfig;
use opencli_api::provider::WireApi;
use opencli_client::ReqwestTransport;
...
async fn models_client_hits_models_endpoint() {
    ...
    let base_url = format!("{}/api/opencli", server.uri());
    ...
    assert_eq!(received[0].url.path(), "/api/opencli/models");
}

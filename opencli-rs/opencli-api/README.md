...
Typed clients for OpenCLI/OpenAI APIs built on top of the generic transport in `opencli-client`.
...
- Serves as the wire-level layer consumed by `opencli-core`; higher layers handle auth refresh and business logic.
...
- **Prompted endpoints (Chat + Responses)**
  - Input: a single `Prompt` plus endpoint-specific options.
    - `Prompt` (re-exported as `opencli_api::Prompt`) carries:
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `opencli_api::CompactionInput`):
    - `model: &str`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `opencli-api` and `opencli-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.

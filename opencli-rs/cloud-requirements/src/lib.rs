//! Cloud-hosted config requirements for OpenCLI.
//!
...
//! customers.
//!
//! Today, fetching is best-effort: on error or timeout, OpenCLI continues without cloud requirements.
//! We expect to tighten this so that Enterprise ChatGPT customers must successfully fetch these
//! requirements before OpenCLI will run.
...
/// This blocks opencli startup, so must be short.
...
trait RequirementsFetcher: Send + Sync {
    ...
    /// Result.
    async fn fetch_requirements(&self, auth: &OpenCLIAuth) -> Option<String>;
}
...
impl CloudRequirementsService {
    ...
    async fn fetch_with_timeout(&self) -> Option<ConfigRequirementsToml> {
        let _timer =
            opencli_otel::start_global_timer("opencli.cloud_requirements.fetch.duration_ms", &[]);
...
mod tests {
    ...
    impl RequirementsFetcher for StaticFetcher {
        async fn fetch_requirements(&self, _auth: &OpenCLIAuth) -> Option<String> {
            self.contents.clone()
        }
    }

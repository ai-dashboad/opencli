...
use opencli_core::config::types::ShellEnvironmentPolicy;
use opencli_core::error::OpenCLIErr;
use opencli_core::error::SandboxErr;
use opencli_core::exec::ExecParams;
use opencli_core::exec::process_exec_tool_call;
use opencli_core::exec_env::create_env;
use opencli_core::protocol::SandboxPolicy;
use opencli_core::protocol_config_types::WindowsSandboxLevel;
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> opencli_core::exec::ExecToolCallOutput {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_opencli-linux-sandbox");
    let opencli_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_opencli-linux-sandbox");
    let opencli_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
    ...
    let output = match result {
        Ok(output) => output,
        Err(OpenCLIErr::Sandbox(SandboxErr::Denied { output })) => *output,
        _ => {

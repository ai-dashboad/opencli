...
use opencli_chatgpt::apply_command::ApplyCommand;
use opencli_chatgpt::apply_command::run_apply_command;
use opencli_cli::LandlockCommand;
use opencli_cli::SeatbeltCommand;
use opencli_cli::WindowsCommand;
use opencli_cli::login::read_api_key_from_stdin;
use opencli_cli::login::run_login_status;
use opencli_cli::login::run_login_with_api_key;
use opencli_cli::login::run_login_with_chatgpt;
use opencli_cli::login::run_login_with_device_code;
use opencli_cli::login::run_logout;
use opencli_cloud_tasks::Cli as CloudTasksCli;
use opencli_common::CliConfigOverrides;
use opencli_exec::Cli as ExecCli;
use opencli_exec::Command as ExecCommand;
use opencli_exec::ReviewArgs;
use opencli_execpolicy::ExecPolicyCheckCommand;
use opencli_responses_api_proxy::Args as ResponsesApiProxyArgs;
use opencli_tui::AppExitInfo;
use opencli_tui::Cli as TuiCli;
use opencli_tui::ExitReason;
use opencli_tui::update_action::UpdateAction;
...
use opencli_core::features::Stage;
use opencli_core::features::is_known_feature_key;
use opencli_core::terminal::TerminalName;
...
#[clap(
    ...
    // `opencli-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `opencli` command name that users run.
    bin_name = "opencli",
    override_usage = "opencli [OPTIONS] [PROMPT]\n       opencli [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// [experimental] Run OpenCLI as an MCP server and manage MCP servers.
    Mcp(McpCli),

    /// [experimental] Run the OpenCLI MCP server (stdio transport).
    McpServer,
    ...
    /// Run commands within a OpenCLI-provided sandbox.
    ...
    /// Apply the latest diff produced by OpenCLI agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from OpenCLI Cloud and apply changes locally.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | opencli login --with-api-key`)"
    )]
    with_api_key: bool,
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    let mut lines = vec![format!(
        "{}",
        opencli_core::protocol::FinalOutput::from(token_usage)
    )];

    if let Some(resume_cmd) =
        opencli_core::util::resume_command(thread_name.as_deref(), conversation_id)
    {
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating OpenCLI via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart OpenCLI.");
    Ok(())
}
...
fn stage_str(stage: opencli_core::features::Stage) -> &'static str {
    use opencli_core::features::Stage;
    match stage {
...
fn main() -> anyhow::Result<()> {
    arg0_dispatch_or_else(|opencli_linux_sandbox_exe| async move {
        cli_main(opencli_linux_sandbox_exe).await?;
        Ok(())
    })
}
...
async fn cli_main(opencli_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    match subcommand {
        None => {
            ...
            let exit_info = run_interactive_tui(interactive, opencli_linux_sandbox_exe).await?;
        ...
        Some(Subcommand::McpServer) => {
            opencli_mcp_server::run_main(opencli_linux_sandbox_exe, root_config_overrides).await?;
        }
        ...
        Some(Subcommand::AppServer(app_server_cli)) => match app_server_cli.subcommand {
            None => {
                opencli_app_server::run_main(
                    opencli_linux_sandbox_exe,
                    root_config_overrides,
                    opencli_core::config_loader::LoaderOverrides::default(),
                    app_server_cli.analytics_default_enabled,
                )
                .await?;
            }
            Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                opencli_app_server_protocol::generate_ts(
                    &gen_cli.out_dir,
                    gen_cli.prettier.as_deref(),
                )?;
            }
            Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                opencli_app_server_protocol::generate_json(&gen_cli.out_dir)?;
            }
        },
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | opencli login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            opencli_cloud_tasks::run_main(cloud_cli, opencli_linux_sandbox_exe).await?;
        }
        Some(Subcommand::Sandbox(sandbox_args)) => match sandbox_args.cmd {
            SandboxCommand::Macos(mut seatbelt_cli) => {
                ...
                opencli_cli::debug_sandbox::run_command_under_seatbelt(
                    seatbelt_cli,
                    opencli_linux_sandbox_exe,
                )
                .await?;
            ...
            SandboxCommand::Linux(mut landlock_cli) => {
                ...
                opencli_cli::debug_sandbox::run_command_under_landlock(
                    landlock_cli,
                    opencli_linux_sandbox_exe,
                )
                .await?;
            ...
            SandboxCommand::Windows(mut windows_cli) => {
                ...
                opencli_cli::debug_sandbox::run_command_under_windows(
                    windows_cli,
                    opencli_linux_sandbox_exe,
                )
                .await?;
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            tokio::task::spawn_blocking(move || opencli_responses_api_proxy::run_main(args))
                .await??;
        }
        Some(Subcommand::StdioToUds(cmd)) => {
            let socket_path = cmd.socket_path;
            tokio::task::spawn_blocking(move || opencli_stdio_to_uds::run(socket_path.as_path()))
                .await??;
        }
        Some(Subcommand::Features(FeaturesCli { sub })) => match sub {
            FeaturesSubcommand::List => {
                ...
                .await?;
                let mut rows = Vec::with_capacity(opencli_core::features::FEATURES.len());
                let mut name_width = 0;
                let mut stage_width = 0;
                for def in opencli_core::features::FEATURES.iter() {
                    let name = def.key;
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: &str) -> anyhow::Result<()> {
    ...
    let opencli_home = find_opencli_home()?;
    ...
    maybe_print_under_development_feature_warning(&opencli_home, interactive, feature);
    Ok(())
}
...
fn maybe_print_under_development_feature_warning(
    ...
    let Some(spec) = opencli_core::features::FEATURES
        .iter()
    ...
    let config_path = opencli_home.join(opencli_core::config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_interactive_tui(
    ...
    let terminal_info = opencli_core::terminal::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". OpenCLI's interactive TUI may not work in this terminal."
        );
    ...
    opencli_tui::run_main(interactive, opencli_linux_sandbox_exe).await
}
...
/// Build the final `TuiCli` for a `opencli resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `opencli` without additional flags.
...
/// Build the final `TuiCli` for a `opencli fork` invocation.
...
/// Merge flags provided to `opencli resume`/`opencli fork` so they take precedence over any
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["opencli", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(opencli_exec::Command::Resume(args)) = exec.command else {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run opencli resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run opencli resume my-thread".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["opencli", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["opencli", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["opencli", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["opencli", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["opencli", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_merges_option_flags_and_full_auto() {
        ...
        assert_matches!(
            ...
            Some(opencli_common::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(opencli_common::ApprovalModeCliArg::OnRequest)
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["opencli", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["opencli", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["opencli", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["opencli", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["opencli", "app-server"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["opencli", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["opencli", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["opencli", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };

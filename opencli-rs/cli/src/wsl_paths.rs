...
pub use opencli_core::env::is_wsl;
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\opencli.zip").as_deref(),
            Some("/mnt/c/Temp/opencli.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/opencli.tgz").as_deref(),
            Some("/mnt/d/Work/opencli.tgz")
        );
        assert!(win_path_to_wsl("/home/user/opencli").is_none());
    }

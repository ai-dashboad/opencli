...
use opencli_core::auth::logout;
use opencli_core::config::Config;
use opencli_login::ServerOptions;
use opencli_login::run_device_code_login;
...
pub fn read_api_key_from_stdin() -> String {
    ...
    if stdin.is_terminal() {
        eprintln!(
            "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | opencli login --with-api-key`."
        );
        std::process::exit(1);
    }
...
/// `opencli login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match OpenCLIAuth::from_auth_storage(&config.opencli_home, config.cli_auth_credentials_store_mode) {
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout(&config.opencli_home, config.cli_auth_credentials_store_mode) {
        Ok(true) => {

...
use opencli_core::config::load_global_mcp_servers;
use opencli_core::config::types::McpServerConfig;
use opencli_core::config::types::McpServerTransportConfig;
use opencli_core::mcp::auth::McpOAuthLoginSupport;
use opencli_core::mcp::auth::compute_auth_statuses;
use opencli_core::mcp::auth::oauth_login_support;
use opencli_core::protocol::McpAuthStatus;
use opencli_rmcp_client::delete_oauth_tokens;
use opencli_rmcp_client::perform_oauth_login;

/// Subcommands:
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.opencli/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "opencli mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let opencli_home = find_opencli_home().context("failed to resolve OPENCLI_HOME")?;
    let mut servers = load_global_mcp_servers(&opencli_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", opencli_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `opencli mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `opencli mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: opencli mcp remove {}", get_args.name);

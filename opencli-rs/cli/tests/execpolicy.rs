...
fn execpolicy_check_matches_expected_json() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let policy_path = opencli_home.path().join("rules").join("policy.rules");
    fs::create_dir_all(
        policy_path
            .parent()
...
"#,
    ...
    let output = Command::new(opencli_utils_cargo_bin::cargo_bin("opencli")?)
        .env("OPENCLI_HOME", opencli_home.path())
        .args([
            "execpolicy",
            "check",
            "--rules",

...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = opencli_command(opencli_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(opencli_home.path()).await?;
    ...
    let mut list_cmd = opencli_command(opencli_home.path())?;
    ...
    let mut list_json_cmd = opencli_command(opencli_home.path())?;
    ...
    let mut get_cmd = opencli_command(opencli_home.path())?;
    ...
    assert!(stdout.contains("remove: opencli mcp remove docs"));

    let mut get_json_cmd = opencli_command(opencli_home.path())?;
    get_json_cmd

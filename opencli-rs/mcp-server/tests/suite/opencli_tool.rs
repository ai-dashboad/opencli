...
use opencli_core::parse_command;
use opencli_core::protocol::FileChange;
use opencli_core::protocol::ReviewDecision;
use opencli_core::spawn::OPENCLI_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use opencli_mcp_server::ExecApprovalElicitRequestParams;
use opencli_mcp_server::ExecApprovalResponse;
use opencli_mcp_server::OpenCLIToolCallParam;
use opencli_mcp_server::PatchApprovalElicitRequestParams;
use opencli_mcp_server::PatchApprovalResponse;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(OPENCLI_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a OpenCLI sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "opencli" tool request, which should hit the completions endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let opencli_request_id = mcp_process
        .send_opencli_tool_call(OpenCLIToolCallParam {
    ...
    let expected_elicitation_request = create_expected_elicitation_request(
        ...
        opencli_request_id.to_string(),
        params.opencli_event_id.clone(),
        params.thread_id,
    )?;
    ...
    // Verify the original `opencli` tool call completes and that the file was created.
    let opencli_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(opencli_request_id)),
    )
    .await??;
    assert_eq!(
        JSONRPCResponse {
            jsonrpc: JSONRPC_VERSION.into(),
            id: RequestId::Integer(opencli_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request(
    ...
    command: Vec<String>,
    workdir: &Path,
    opencli_mcp_tool_call_id: String,
    opencli_event_id: String,
    thread_id: opencli_protocol::ThreadId,
) -> anyhow::Result<JSONRPCRequest> {
    let expected_message = format!(
        "Allow OpenCLI to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let opencli_parsed_cmd = parse_command::parse_command(&command);
    Ok(JSONRPCRequest {
        jsonrpc: JSONRPC_VERSION.into(),
        ...
        params: Some(serde_json::to_value(&ExecApprovalElicitRequestParams {
            ...
            thread_id,
            opencli_elicitation: "exec-approval".to_string(),
            opencli_mcp_tool_call_id,
            opencli_event_id,
            opencli_command: command,
            opencli_cwd: workdir.to_path_buf(),
            opencli_call_id: "call1234".to_string(),
            opencli_parsed_cmd,
        })?),
    })
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "opencli" tool request that will trigger the apply_patch command
    let opencli_request_id = mcp_process
        .send_opencli_tool_call(OpenCLIToolCallParam {
    ...
    // Verify the original `opencli` tool call completes
    let opencli_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(opencli_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_opencli_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = opencli_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn opencli_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `opencli mcp` with a specific config.toml.
    let opencli_home = TempDir::new()?;
    create_config_toml(opencli_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(opencli_home.path()).await?;
    ...
    .await??;
    assert_eq!(opencli_response.jsonrpc, JSONRPC_VERSION);
    assert_eq!(opencli_response.id, RequestId::Integer(opencli_request_id));
    assert_eq!(
        opencli_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": opencli_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("opencli tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request(
    ...
    message_lines.push("Allow OpenCLI to apply proposed code changes?".to_string());
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&PatchApprovalElicitRequestParams {
            ...
            thread_id,
            opencli_elicitation: "patch-approval".to_string(),
            opencli_mcp_tool_call_id,
            opencli_event_id,
            opencli_reason: reason,
            opencli_grant_root: grant_root,
            opencli_changes: changes,
            opencli_call_id: "call1234".to_string(),
        })?),
    })
}
...
/// Create a OpenCLI config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(opencli_home: &Path, server_uri: &str) -> std::io::Result<()> {

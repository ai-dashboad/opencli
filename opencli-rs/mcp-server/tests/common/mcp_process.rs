...
impl McpProcess {
    ...
    pub async fn new_with_env(
        ...
        let program = opencli_utils_cargo_bin::cargo_bin("opencli-mcp-server")
            .context("should find binary for opencli-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = opencli_core::default_client::originator().value;
    ...
    pub async fn send_opencli_tool_call(
        &mut self,
        params: OpenCLIToolCallParam,
    ) -> anyhow::Result<i64> {
        let opencli_tool_call_params = CallToolRequestParams {
            name: "opencli".to_string(),
            arguments: Some(serde_json::to_value(params)?),
        };
        self.send_request(
            mcp_types::CallToolRequest::METHOD,
            Some(serde_json::to_value(opencli_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "opencli/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JSONRPCMessage::Notification(notification) => {
                    let is_match = if notification.method == "opencli/event" {

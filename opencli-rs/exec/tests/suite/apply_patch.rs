...
/// While we may add an `apply-patch` subcommand to the `opencli` CLI multitool
/// at some point, we must ensure that the smaller `opencli-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(opencli_utils_cargo_bin::cargo_bin("opencli-exec")?)
        .arg(OPENCLI_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch

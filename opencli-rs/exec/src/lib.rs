...
use opencli_protocol::approvals::ElicitationAction;
...
struct ThreadEventEnvelope {
    ...
    thread: Arc<opencli_core::OpenCLIThread>,
    event: Event,
}
...
pub async fn run_main(cli: Cli, opencli_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        opencli_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, false)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
    ...
    let (initial_operation, prompt_summary) = match (command, prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = opencli_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what OpenCLI
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);

    info!("OpenCLI initialized with event: {session_configured:?}");
...

    {
        ...
        tokio::spawn(async move {
            if tokio::signal::ctrl_c().await.is_ok() {
                ...
                // Immediately notify OpenCLI to abort any in-flight task.
    ...
    while let Some(envelope) = rx.recv().await {
        ...
        if thread_id != primary_thread_id && matches!(shutdown, OpenCLIStatus::InitiateShutdown) {
            continue;
        }
        match shutdown {
            OpenCLIStatus::Running => continue,
            OpenCLIStatus::InitiateShutdown => {
                thread.submit(Op::Shutdown).await?;
            }
            OpenCLIStatus::Shutdown if thread_id == primary_thread_id => break,
            OpenCLIStatus::Shutdown => continue,
        }
    }
...
async fn resolve_resume_path(
    ...
    if args.last {
        ...
        match opencli_core::RolloutRecorder::find_latest_thread_path(
            &config.opencli_home,
            1,
            None,
            opencli_core::ThreadSortKey::UpdatedAt,
            &[],
        ...
        if Uuid::parse_str(id_str).is_ok() {
            let path = find_thread_path_by_id_str(&config.opencli_home, id_str).await?;
            Ok(path)
        } else {
            let path = find_thread_path_by_name_str(&config.opencli_home, id_str).await?;
            Ok(path)
        }
    } else {
        Ok(None)

...
use opencli_protocol::approvals::ExecPolicyAmendment as CoreExecPolicyAmendment;
...
use opencli_protocol::config_types::SandboxMode as CoreSandboxMode;
use opencli_protocol::config_types::Verbosity;
use opencli_protocol::config_types::WebSearchMode;
use opencli_protocol::items::AgentMessageContent as CoreAgentMessageContent;
use opencli_protocol::items::TurnItem as CoreTurnItem;
use opencli_protocol::models::ResponseItem;
use opencli_protocol::openai_models::ReasoningEffort;
use opencli_protocol::parse_command::ParsedCommand as CoreParsedCommand;
use opencli_protocol::plan_tool::PlanItemArg as CorePlanItemArg;
use opencli_protocol::plan_tool::StepStatus as CorePlanStepStatus;
use opencli_protocol::protocol::AgentStatus as CoreAgentStatus;
use opencli_protocol::protocol::AskForApproval as CoreAskForApproval;
use opencli_protocol::protocol::OpenCLIErrorInfo as CoreOpenCLIErrorInfo;
use opencli_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use opencli_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use opencli_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use opencli_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use opencli_protocol::protocol::SessionSource as CoreSessionSource;
use opencli_protocol::protocol::SkillDependencies as CoreSkillDependencies;
use opencli_protocol::protocol::SkillErrorInfo as CoreSkillErrorInfo;
use opencli_protocol::protocol::SkillInterface as CoreSkillInterface;
use opencli_protocol::protocol::SkillMetadata as CoreSkillMetadata;
use opencli_protocol::protocol::SkillScope as CoreSkillScope;
use opencli_protocol::protocol::SkillToolDependency as CoreSkillToolDependency;
use opencli_protocol::protocol::SubAgentSource as CoreSubAgentSource;
use opencli_protocol::protocol::TokenUsage as CoreTokenUsage;
use opencli_protocol::protocol::TokenUsageInfo as CoreTokenUsageInfo;
use opencli_protocol::user_input::ByteRange as CoreByteRange;
use opencli_protocol::user_input::TextElement as CoreTextElement;
use opencli_protocol::user_input::UserInput as CoreUserInput;
...
/// This translation layer make sure that we expose opencli error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `opencliErrorInfo` variant.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub enum OpenCLIErrorInfo {
...
impl From<CoreOpenCLIErrorInfo> for OpenCLIErrorInfo {
    fn from(value: CoreOpenCLIErrorInfo) -> Self {
        match value {
            CoreOpenCLIErrorInfo::ContextWindowExceeded => OpenCLIErrorInfo::ContextWindowExceeded,
            CoreOpenCLIErrorInfo::UsageLimitExceeded => OpenCLIErrorInfo::UsageLimitExceeded,
            CoreOpenCLIErrorInfo::ModelCap {
                model,
                reset_after_seconds,
            ...
            CoreOpenCLIErrorInfo::HttpConnectionFailed { http_status_code } => {
                OpenCLIErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreOpenCLIErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                OpenCLIErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreOpenCLIErrorInfo::InternalServerError => OpenCLIErrorInfo::InternalServerError,
            CoreOpenCLIErrorInfo::Unauthorized => OpenCLIErrorInfo::Unauthorized,
            CoreOpenCLIErrorInfo::BadRequest => OpenCLIErrorInfo::BadRequest,
            CoreOpenCLIErrorInfo::ThreadRollbackFailed => OpenCLIErrorInfo::ThreadRollbackFailed,
            CoreOpenCLIErrorInfo::SandboxError => OpenCLIErrorInfo::SandboxError,
            CoreOpenCLIErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                OpenCLIErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreOpenCLIErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                OpenCLIErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreOpenCLIErrorInfo::Other => OpenCLIErrorInfo::Other,
        }
    }
}
...
v2_enum_from_core!(
    pub enum ReviewDelivery from opencli_protocol::protocol::ReviewDelivery {
        Inline, Detached
    }
);

v2_enum_from_core!(
    pub enum McpAuthStatus from opencli_protocol::protocol::McpAuthStatus {
        Unsupported,
        NotLoggedIn,
        BearerToken,
        OAuth
...
pub enum ConfigLayerSource {
    ...
    /// User config layer from $OPENCLI_HOME/config.toml. This layer is special
    ...
    /// Path to a .opencli/ folder within a project. There could be multiple of
    /// these between `cwd` and the project/repo root.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    Project {
        dot_opencli_folder: AbsolutePathBuf,
    },
...
impl SandboxPolicy {
    pub fn to_core(&self) -> opencli_protocol::protocol::SandboxPolicy {
        match self {
            SandboxPolicy::DangerFullAccess => {
                opencli_protocol::protocol::SandboxPolicy::DangerFullAccess
            }
            SandboxPolicy::ReadOnly => opencli_protocol::protocol::SandboxPolicy::ReadOnly,
            SandboxPolicy::ExternalSandbox { network_access } => {
                opencli_protocol::protocol::SandboxPolicy::ExternalSandbox {
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                exclude_slash_tmp,
            } => opencli_protocol::protocol::SandboxPolicy::WorkspaceWrite {
...
impl From<opencli_protocol::protocol::SandboxPolicy> for SandboxPolicy {
    fn from(value: opencli_protocol::protocol::SandboxPolicy) -> Self {
        match value {
            opencli_protocol::protocol::SandboxPolicy::DangerFullAccess => {
                SandboxPolicy::DangerFullAccess
            }
            opencli_protocol::protocol::SandboxPolicy::ReadOnly => SandboxPolicy::ReadOnly,
            opencli_protocol::protocol::SandboxPolicy::ExternalSandbox { network_access } => {
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that OpenCLI-managed ChatGPT auth tokens have.
...
pub enum ChatgptAuthTokensRefreshReason {
    /// OpenCLI attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that OpenCLI was previously using.
    ///
...
pub struct ThreadStartParams {
    ...
    ///
    /// This is for internal use only (e.g. OpenCLI Cloud).
...
pub struct ThreadResumeParams {
    ...
    /// [UNSTABLE] FOR OPENCLI CLOUD - DO NOT USE.
...
pub struct TurnError {
    pub message: String,
    pub opencli_error_info: Option<OpenCLIErrorInfo>,
    #[serde(default)]
...
impl From<opencli_protocol::models::WebSearchAction> for WebSearchAction {
    fn from(value: opencli_protocol::models::WebSearchAction) -> Self {
        match value {
            opencli_protocol::models::WebSearchAction::Search { query, queries } => {
                WebSearchAction::Search { query, queries }
            }
            opencli_protocol::models::WebSearchAction::OpenPage { url } => {
                WebSearchAction::OpenPage { url }
            }
            opencli_protocol::models::WebSearchAction::FindInPage { url, pattern } => {
                WebSearchAction::FindInPage { url, pattern }
            }
            opencli_protocol::models::WebSearchAction::Other => WebSearchAction::Other,
        }
    }
}
...
mod tests {
    ...
    use opencli_protocol::items::AgentMessageItem;
    use opencli_protocol::items::ReasoningItem;
    use opencli_protocol::items::TurnItem;
    use opencli_protocol::items::UserMessageItem;
    use opencli_protocol::items::WebSearchItem;
    use opencli_protocol::models::WebSearchAction as CoreWebSearchAction;
    use opencli_protocol::protocol::NetworkAccess as CoreNetworkAccess;
    use opencli_protocol::user_input::UserInput as CoreUserInput;
        ...
        let user_item = TurnItem::UserMessage(UserMessageItem {
            ...
            content: vec![
                ...
                CoreUserInput::Skill {
                    ...
                    path: PathBuf::from("/repo/.opencli/skills/skill-creator/SKILL.md"),
                },
    ...
    #[test]
    fn opencli_error_info_serializes_http_status_code_in_camel_case() {
        let value = OpenCLIErrorInfo::ResponseTooManyFailedAttempts {

...
This repository uses Bazel to build the Rust workspace under `opencli-rs`.
...
- `rules_rs` imports third-party crates from `opencli-rs/Cargo.toml` and
  `opencli-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `opencli_rust_crate`, which wraps `rust_library`,
...
- Each crate in `opencli-rs/*/BUILD.bazel` typically uses `opencli_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `opencli_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `opencli_rust_crate` to configure it.

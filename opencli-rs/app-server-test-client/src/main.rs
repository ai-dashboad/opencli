...
use opencli_app_server_protocol::InitializeParams;
use opencli_app_server_protocol::InitializeResponse;
use opencli_app_server_protocol::InputItem;
use opencli_app_server_protocol::JSONRPCMessage;
...
use opencli_app_server_protocol::SandboxPolicy;
...
use opencli_app_server_protocol::ServerRequest;
...
/// Minimal launcher that initializes the OpenCLI app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "OpenCLI", version, about = "Bootstrap OpenCLI app-server", long_about = None)]
struct Cli {
    /// Path to the `opencli` CLI binary.
    #[arg(long, env = "OPENCLI_BIN", default_value = "opencli")]
    opencli_bin: String,

    /// Forwarded to the `opencli` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Send a user message through the OpenCLI app-server.
    SendMessage {
        /// User message to send to OpenCLI.
        #[arg()]
        user_message: String,
    },
    ...
    TestLogin,
    /// Fetch the current account rate limits from the OpenCLI app-server.
    GetAccountRateLimits,
    /// List the available models from the OpenCLI app-server.
...
fn main() -> Result<()> {
    ...
    match command {
        CliCommand::SendMessage { user_message } => {
            send_message(&opencli_bin, &config_overrides, user_message)
        }
        CliCommand::SendMessageV2 { user_message } => {
            send_message_v2(&opencli_bin, &config_overrides, user_message)
        }
        CliCommand::TriggerCmdApproval { user_message } => {
            trigger_cmd_approval(&opencli_bin, &config_overrides, user_message)
        }
...
fn send_message_v2(
    ...
    send_message_v2_with_policies(opencli_bin, config_overrides, user_message, None, None)
}
...
fn test_login(opencli_bin: &str, config_overrides: &[String]) -> Result<()> {
    let mut client = OpenCLIClient::spawn(opencli_bin, config_overrides)?;
...
fn get_account_rate_limits(opencli_bin: &str, config_overrides: &[String]) -> Result<()> {
    let mut client = OpenCLIClient::spawn(opencli_bin, config_overrides)?;
...
fn model_list(opencli_bin: &str, config_overrides: &[String]) -> Result<()> {
    let mut client = OpenCLIClient::spawn(opencli_bin, config_overrides)?;
...
impl OpenCLIClient {
    fn spawn(opencli_bin: &str, config_overrides: &[String]) -> Result<Self> {
        let mut cmd = Command::new(opencli_bin);
        for override_kv in config_overrides {
            cmd.arg("--config").arg(override_kv);
        }
        let mut opencli_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{opencli_bin}` app-server"))?;

        let stdin = opencli_app_server
            .stdin
            .take()
            .context("opencli app-server stdin unavailable")?;
        let stdout = opencli_app_server
            .stdout
            .take()
            .context("opencli app-server stdout unavailable")?;

        Ok(Self {
            child: opencli_app_server,
            stdin: Some(stdin),
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "opencli-toy-app-server".to_string(),
                    title: Some("OpenCLI Toy App Server".to_string()),
    ...
    fn stream_conversation(&mut self, conversation_id: &ThreadId) -> Result<()> {
        loop {
            ...
            if !notification.method.starts_with("opencli/event/") {
                continue;
            }
    ...
    fn write_request(&mut self, request: &ClientRequest) -> Result<()> {
        ...
        if let Some(stdin) = self.stdin.as_mut() {
            ...
            stdin
                .flush()
                .context("failed to flush request to opencli app-server")?;
        } else {
            bail!("opencli app-server stdin closed");
        }
    ...
    fn read_jsonrpc_message(&mut self) -> Result<JSONRPCMessage> {
        loop {
            ...
            let bytes = self
                ...
                .context("failed to read from opencli app-server")?;

            if bytes == 0 {
                bail!("opencli app-server closed stdout");
            }
    ...
    fn write_jsonrpc_message(&mut self, message: JSONRPCMessage) -> Result<()> {
        ...
        if let Some(stdin) = self.stdin.as_mut() {
            ...
            stdin
                .flush()
                .context("failed to flush response to opencli app-server")?;
            return Ok(());
        }
...
impl Drop for OpenCLIClient {
    fn drop(&mut self) {
        let _ = self.stdin.take();

        if let Ok(Some(status)) = self.child.try_wait() {
            println!("[opencli app-server exited: {status}]");
            return;
        }

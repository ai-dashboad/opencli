...
const MEMO_URI: &str = "memo://opencli/example-note";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of OpenCLI TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (OpenCLI TUI):
    /// - Build this binary: `cargo build -p opencli-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `opencli mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in OpenCLI TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://opencli/{slug}".to_string(),
            name: "opencli-memo".to_string(),
            title: Some("OpenCLI Memo".to_string()),
            description: Some(
                "Template for memo://opencli/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
        };
...
/// Scenarios for `image_scenario`, intended to exercise OpenCLI TUI handling of MCP image outputs.
///

...
//!
//! If the keyring is not available or fails, we fall back to OPENCLI_HOME/.credentials.json which is consistent with other coding CLI agents.
...
use opencli_utils_home_dir::find_opencli_home;

const KEYRING_SERVICE: &str = "OpenCLI MCP Credentials";
...
/// Determine where OpenCLI should store and read MCP credentials.
...
fn fallback_file_path() -> Result<PathBuf> {
    let mut path = find_opencli_home()?;
...
mod tests {
    ...
    struct TempOpenCLIHome {
        _guard: MutexGuard<'static, ()>,
        _dir: tempfile::TempDir,
    }

    impl TempOpenCLIHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create OPENCLI_HOME temp dir");
            unsafe {
                std::env::set_var("OPENCLI_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempOpenCLIHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("OPENCLI_HOME");
            }
        }
    }

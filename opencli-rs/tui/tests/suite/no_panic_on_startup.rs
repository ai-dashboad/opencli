...
"#,
    ...
    std::fs::write(opencli_home.join("config.toml"), config_contents)?;

    let OpenCLICliOutput { exit_code, output } = run_opencli_cli(opencli_home, cwd).await?;
    assert_ne!(0, exit_code, "OpenCLI CLI should exit nonzero.");
    assert!(
        output.contains("ERROR: Failed to initialize opencli:"),
...
struct OpenCLICliOutput {
    exit_code: i32,
    output: String,
}

async fn run_opencli_cli(
    opencli_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<OpenCLICliOutput> {
    let opencli_cli = opencli_utils_cargo_bin::cargo_bin("opencli")?;
    let mut env = HashMap::new();
    env.insert(
        "OPENCLI_HOME".to_string(),
        opencli_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = opencli_utils_pty::spawn_pty_process(
        opencli_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            ...
            anyhow::bail!("timed out waiting for opencli CLI to exit");
        }
    };

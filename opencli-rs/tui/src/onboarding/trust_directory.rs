...
impl WidgetRef for &TrustDirectoryWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        column.push(Line::from(vec![
            "> ".into(),
            "You are running OpenCLI in ".bold(),
            self.cwd.to_string_lossy().to_string().into(),
        ]));
        column.push("");

        let guidance = if self.is_git_repo {
            "Since this folder is version controlled, you may wish to allow OpenCLI to work in this folder without asking for approval."
        } else {
        ...
        if self.is_git_repo {
            options.push((
                "Yes, allow OpenCLI to work in this folder without asking for approval",
            ...
            options.push((
                "Allow OpenCLI to work in this folder without asking for approval",
                TrustDirectorySelection::Trust,
            ));
            options.push((
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trust_level(&self.opencli_home, &target, TrustLevel::Trusted) {
...
mod tests {
    ...
    fn release_event_does_not_change_selection() {
        let opencli_home = TempDir::new().expect("temp home");

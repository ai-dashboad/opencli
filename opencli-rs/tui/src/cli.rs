use clap::Parser;
use clap::ValueHint;
use opencli_common::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `opencli resume` subcommand.
    // These are not exposed as user flags on the base `opencli` command.
    #[clap(skip)]
    ...
    /// top-level `opencli resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `opencli fork` subcommand.
    // These are not exposed as user flags on the base `opencli` command.
    #[clap(skip)]
    ...
    /// top-level `opencli fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    pub sandbox_mode: Option<opencli_common::SandboxModeCliArg>,

...
use opencli_core::protocol::FinalOutput;
...
use opencli_core::protocol::SkillErrorInfo;
...
fn session_summary(
    ...
    let resume_command = opencli_core::util::resume_command(thread_name.as_deref(), thread_id);
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    {
        let ConfigLayerSource::Project { dot_opencli_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_opencli_folder.as_path().display().to_string(),
            layer
                .disabled_reason
                .as_ref()
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        &self,
        tui: &mut tui::Tui,
        cfg: opencli_core::config::Config,
    ...
    async fn rebuild_config_for_cwd(&self, cwd: PathBuf) -> Result<Config> {
        ...
        ConfigBuilder::default()
            .opencli_home(self.config.opencli_home.clone())
    ...
    async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
        ...
        let opencli_op_tx = crate::chatwidget::spawn_op_forwarder(thread);
        self.chat_widget = ChatWidget::new_with_op_sender(init, opencli_op_tx);
    ...
    async fn drain_active_thread_events(&mut self, tui: &mut tui::Tui) -> Result<()> {
        ...
        loop {
            match rx.try_recv() {
                Ok(event) => self.handle_opencli_event_now(event),
    ...
    fn replay_thread_snapshot(&mut self, snapshot: ThreadEventSnapshot) {
        if let Some(event) = snapshot.session_configured {
            self.handle_opencli_event_replay(event);
        }
    ...
    pub async fn run(
        ...
        let feedback_audience = if auth_ref
            .and_then(OpenCLIAuth::get_account_email)
        ...
        let otel_manager = OtelManager::new(
            ...
            model.as_str(),
            model.as_str(),
            auth_ref.and_then(OpenCLIAuth::get_account_id),
            auth_ref.and_then(OpenCLIAuth::get_account_email),
            auth_ref.map(OpenCLIAuth::api_auth_mode),
            config.otel.log_user_prompt,
            opencli_core::terminal::user_agent(),
            SessionSource::Cli,
        );
        ...
        #[cfg(target_os = "windows")]
        {
            let should_check = WindowsSandboxLevel::from_config(&app.config)
                ...
                && matches!(
                    ...
                    opencli_core::protocol::SandboxPolicy::WorkspaceWrite { .. }
                        | opencli_core::protocol::SandboxPolicy::ReadOnly
                )
                && !app
                    .config
                    .notices
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.opencli_home.clone();
        ...
        match event {
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let opencli_home = self.config.opencli_home.clone();
                    let tx = self.app_event_tx.clone();
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = opencli_core::windows_sandbox::run_elevated_setup(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                        ...
                        let event = match result {
                            Ok(()) => {
                                otel_manager.counter(
                                    "opencli.windows_sandbox.elevated_setup_success",
                                    1,
                                    &[],
                                );
                            ...
                            Err(err) => {
                                ...
                                if let Some((code, message)) =
                                    opencli_core::windows_sandbox::elevated_setup_failure_details(
                                        &err,
                                    )
                                {
                                    code_tag = Some(code);
                                ...
                                otel_manager.counter(
                                    "opencli.windows_sandbox.elevated_setup_failure",
                                    1,
                                    &tags,
                                );
                                tracing::error!(
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let mut builder =
                        ConfigEditsBuilder::new(&self.config.opencli_home).with_profile(profile);
                    if elevated_enabled {
                    ...
                    match builder.apply().await {
                        Ok(()) => {
                            ...
                            {
                                self.app_event_tx.send(AppEvent::OpenCLIOp(
            ...
            AppEvent::UpdateSandboxPolicy(policy) => {
                ...
                #[cfg(target_os = "windows")]
                if !matches!(&policy, opencli_core::protocol::SandboxPolicy::ReadOnly)
                    || WindowsSandboxLevel::from_config(&self.config)
                        != WindowsSandboxLevel::Disabled
                {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.opencli_home.clone();
            ...
            AppEvent::UpdateFeatureFlags { updates } => {
                ...
                let mut builder = ConfigEditsBuilder::new(&self.config.opencli_home)
    ...
    fn handle_opencli_event_now(&mut self, event: Event) {
        ...
        self.chat_widget.handle_opencli_event(event);
    }

    fn handle_opencli_event_replay(&mut self, event: Event) {
        self.handle_backtrack_event(&event.msg);
        self.chat_widget.handle_opencli_event_replay(event);
    }

    fn handle_active_thread_event(&mut self, tui: &mut tui::Tui, event: Event) -> Result<()> {
        self.handle_opencli_event_now(event);
        if self.backtrack_render_pending {
            tui.frame_requester().schedule_frame();
        }
        Ok(())
    ...
    fn reasoning_label_for(
        ...
        (!model.starts_with("opencli-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }

    pub(crate) fn token_usage(&self) -> opencli_core::protocol::TokenUsage {
        self.chat_widget.token_usage()
    }
        ...
        let editor_cmd = match external_editor::resolve_editor_command() {
            ...
            Err(external_editor::EditorError::MissingEditor) => {
                self.chat_widget
                    ...
                    "Cannot open external editor: set $VISUAL or $EDITOR before starting OpenCLI."
                        .to_string(),
                ));
    ...
    fn spawn_world_writable_scan(
        ...
        sandbox_policy: opencli_core::protocol::SandboxPolicy,
        tx: AppEventSender,
    ) {
        tokio::task::spawn_blocking(move || {
            let result = opencli_windows_sandbox::apply_world_writable_scan_and_denies(
                &logs_base_dir,
                &cwd,
                &env_map,
                &sandbox_policy,
...
mod tests {
    ...
    async fn make_test_app() -> App {
        ...
        let server = Arc::new(ThreadManager::with_models_provider(
            OpenCLIAuth::from_api_key("Test API Key"),
    ...
    async fn backtrack_selection_with_duplicate_history_targets_unique_turn() {
        ...
        app.chat_widget.handle_opencli_event(Event {
            id: String::new(),
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                session_id: base_id,
                forked_from_id: None,
    ...
    async fn session_summary_includes_resume_hint() {
        ...
        assert_eq!(
            ...
            Some("opencli resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    async fn session_summary_prefers_name_over_id() {
        ...
        assert_eq!(
            summary.resume_command,
            Some("opencli resume my-session".to_string())
        );
    }
}

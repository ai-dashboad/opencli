...
//!
//! These tests treat the widget as the adapter between `opencli_core::protocol::EventMsg` inputs and
...
use opencli_core::config::ConstraintError;
use opencli_core::config_loader::RequirementSource;
...
use opencli_core::protocol::ExecPolicyAmendment;
...
use opencli_core::protocol::TurnStartedEvent;
...
async fn resumed_initial_messages_render_history() {
    ...
    let configured = opencli_core::protocol::SessionConfiguredEvent {
...
async fn interrupted_turn_restores_queued_messages_with_images_and_elements() {
    ...
    chat.handle_opencli_event(Event {
        id: "interrupt".into(),
        msg: EventMsg::TurnAborted(opencli_core::protocol::TurnAbortedEvent {
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let thread_manager = Arc::new(ThreadManager::with_models_provider(
        OpenCLIAuth::from_api_key("test"),
        cfg.model_provider.clone(),
    ));
    let auth_manager = AuthManager::from_auth_for_testing(OpenCLIAuth::from_api_key("test"));
    let init = ChatWidgetInit {
        ...
        feedback: opencli_feedback::OpenCLIFeedback::new(),
        is_first_run: true,
...
async fn make_chatwidget_manual(
    ...
    let mut bottom = BottomPane::new(BottomPaneParams {
        ...
        enhanced_keys_supported: false,
        placeholder_text: "Ask OpenCLI to do anything".to_string(),
    ...
    let auth_manager = AuthManager::from_auth_for_testing(OpenCLIAuth::from_api_key("test"));
    let opencli_home = cfg.opencli_home.clone();
    let models_manager = Arc::new(ModelsManager::new(opencli_home, auth_manager.clone()));
...
fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.models_manager = Arc::new(ModelsManager::new(
        chat.config.opencli_home.clone(),
...
async fn rate_limit_switch_prompt_shows_once_per_session() {
    let auth = OpenCLIAuth::create_dummy_chatgpt_auth_for_testing();
...
fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> = opencli_core::parse_command::parse_command(&command);
...
async fn exec_history_cell_shows_working_then_failed() {
    ...
    let blob = lines_to_single_string(lines);
    assert!(
        blob.contains("⏺ Executed") && blob.contains("false"),
...
async fn exec_end_without_begin_uses_event_command() {
    ...
    let parsed_cmd = opencli_core::parse_command::parse_command(&command);
    let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    chat.handle_opencli_event(Event {
    ...
    assert!(
        blob.contains("⏺ Executed") && blob.contains("echo orphaned"),
...
async fn exec_history_shows_unified_exec_startup_commands() {
    ...
    let blob = lines_to_single_string(&cells[0]);
    assert!(
        blob.contains("⏺ Executed") && blob.contains("echo unified exec startup"),
...
async fn unified_exec_wait_status_header_updates_on_late_command_display() {
    ...
    assert_eq!(
        ...
        "Waiting for background task · sleep 5"
    );
}
...
async fn unified_exec_waiting_multiple_empty_snapshots() {
    ...
    assert_eq!(
        chat.current_status_header,
        "Waiting for background task · just fix"
    );
...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no OpenCLI op to be sent, got {other:?}"),
    }
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    let rollout_path = PathBuf::from("/tmp/opencli-test-rollout.jsonl");
...
async fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let entries = vec![
        opencli_core::git_info::CommitLogEntry {
...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::OpenCLIOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::OpenCLIOp(Op::Review { review_request }) => {
            assert_eq!(
                review_request,
                ReviewRequest {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::OpenCLIOp should be sent
...
async fn approvals_popup_navigation_skips_disabled() {
    ...
    assert!(
        app_events.iter().any(|ev| matches!(
            ev,
            AppEvent::OpenCLIOp(Op::OverrideTurnContext {
...
//
// Synthesizes a OpenCLI ExecApprovalRequest event to trigger the approval modal
...
async fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    // Expect a OpenCLIOp with PatchApproval carrying the submission id, not call id
    let mut found = false;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::OpenCLIOp(Op::PatchApproval { id, decision }) = app_ev {
            assert_eq!(id, "sub-123");
            assert_matches!(decision, opencli_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 2) User approves via 'y' and App receives a OpenCLIOp
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));
    let mut maybe_op: Option<Op> = None;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::OpenCLIOp(op) = app_ev {
            maybe_op = Some(op);
            break;
        }
    }
    let op = maybe_op.expect("expected OpenCLIOp after key press");

    // 3) App forwards to widget.submit_op, which pushes onto opencli_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to opencli channel");
    match forwarded {
...
async fn stream_recovery_restores_previous_status_header() {
    ...
    let status = chat
        ...
        .status_widget()
        .expect("status indicator should be visible");
    assert_eq!(status.header(), "Processing");

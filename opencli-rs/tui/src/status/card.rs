...
use chrono::DateTime;
use chrono::Local;
use opencli_common::summarize_sandbox_policy;
use opencli_core::WireApi;
use opencli_core::config::Config;
use opencli_core::protocol::NetworkAccess;
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            ...
            Span::from(" ").dim(),
            Span::from(format!("(v{OPENCLI_CLI_VERSION})")).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run opencli login to use ChatGPT)".to_string()
            }
        });
        ...
        let note_first_line = Line::from(vec![
            ...
            "https://chatgpt.com/opencli/settings/usage"
                .cyan()
                .underlined(),

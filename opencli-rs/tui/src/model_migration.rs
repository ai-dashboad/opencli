...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!(
        "OpenCLI just got an upgrade. Introducing {target_display_name}."
    ))
    .bold();
...
impl ModelMigrationScreen {
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph::new("Choose how you'd like OpenCLI to proceed.")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(0, 2, 0, 0)),
        );
        column.push(Line::from(""));
...
mod tests {
    ...
    fn prompt_snapshot() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                ...
                None,
                Some(
                    "Upgrade to gpt-5.2-opencli for the latest and greatest agentic coding model."
                        .to_string(),
                ),
                None,
                "gpt-5.1-opencli-max".to_string(),
                Some("OpenCLI-optimized flagship for deep and fast reasoning.".to_string()),
                true,
            ),
        );
    ...
    fn prompt_snapshot_gpt5_family() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5",
                "gpt-5.1",
                Some("https://www.opencli.com/models/gpt-5.1".to_string()),
                None,
                None,
                "gpt-5.1".to_string(),
    ...
    #[test]
    fn prompt_snapshot_gpt5_opencli() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-opencli",
                "gpt-5.1-opencli-max",
                Some("https://www.opencli.com/models/gpt-5.1-opencli-max".to_string()),
                None,
                None,
                "gpt-5.1-opencli-max".to_string(),
                Some("OpenCLI-optimized flagship for deep and fast reasoning.".to_string()),
        ...
        terminal.flush().expect("flush");
        assert_snapshot!("model_migration_prompt_gpt5_opencli", terminal.backend());
    }

    #[test]
    fn prompt_snapshot_gpt5_opencli_mini() {
        let backend = VT100Backend::new(60, 22);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect::new(0, 0, 60, 22));
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-opencli-mini",
                "gpt-5.1-opencli-mini",
                Some("https://www.opencli.com/models/gpt-5.1-opencli-mini".to_string()),
                None,
                None,
                "gpt-5.1-opencli-mini".to_string(),
                Some("Optimized for opencli. Cheaper, faster, but less capable.".to_string()),
                false,
            ),
        );
        {
        ...
        terminal.flush().expect("flush");
        assert_snapshot!("model_migration_prompt_gpt5_opencli_mini", terminal.backend());
    }
    ...
    fn escape_key_accepts_prompt() {
        let mut screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-old",
                "gpt-new",
                Some("https://www.opencli.com/models/gpt-new".to_string()),
                None,
                None,
                "gpt-new".to_string(),

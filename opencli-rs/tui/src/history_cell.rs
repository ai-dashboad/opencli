//! Transcript/history cells for the OpenCLI TUI.
//!
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let content = text![
            line![
                ...
                " ",
                format!("{OPENCLI_CLI_VERSION} -> {}", self.latest_version).bold(),
            ],
            update_instruction,
            "",
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: opencli_core::protocol::ReviewDecision,
) -> Box<dyn HistoryCell> {
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    "You ".into(),
                    "approved".bold(),
                    " opencli to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for OpenCLI".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what OpenCLI is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`opencli-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `opencli mcp add`.

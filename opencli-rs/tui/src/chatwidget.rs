//! The main OpenCLI TUI chat surface.
//!
...
use opencli_core::config::ConstraintResult;
use opencli_core::config::types::Notifications;
use opencli_core::features::FEATURES;
use opencli_core::features::Feature;
use opencli_core::git_info::current_branch_name;
use opencli_core::git_info::local_git_branches;
use opencli_core::models_manager::manager::ModelsManager;
use opencli_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
use opencli_core::protocol::AgentMessageDeltaEvent;
use opencli_core::protocol::AgentMessageEvent;
use opencli_core::protocol::AgentReasoningDeltaEvent;
use opencli_core::protocol::AgentReasoningEvent;
use opencli_core::protocol::AgentReasoningRawContentDeltaEvent;
...
use opencli_core::protocol::ExecCommandBeginEvent;
use opencli_core::protocol::ExecCommandEndEvent;
use opencli_core::protocol::ExecCommandOutputDeltaEvent;
use opencli_core::protocol::ExecCommandSource;
use opencli_core::protocol::ExitedReviewModeEvent;
use opencli_core::protocol::ListCustomPromptsResponseEvent;
use opencli_core::protocol::ListSkillsResponseEvent;
use opencli_core::protocol::McpListToolsResponseEvent;
use opencli_core::protocol::McpStartupCompleteEvent;
use opencli_core::protocol::McpStartupStatus;
use opencli_core::protocol::McpStartupUpdateEvent;
...
use opencli_core::protocol::SkillMetadata as ProtocolSkillMetadata;
use opencli_core::protocol::StreamErrorEvent;
use opencli_core::protocol::TerminalInteractionEvent;
use opencli_core::protocol::TokenUsage;
use opencli_core::protocol::TokenUsageInfo;
use opencli_core::protocol::TurnAbortReason;
use opencli_core::protocol::TurnCompleteEvent;
use opencli_core::protocol::TurnDiffEvent;
use opencli_core::protocol::UndoCompletedEvent;
use opencli_core::protocol::UndoStartedEvent;
use opencli_core::protocol::UserMessageEvent;
use opencli_core::protocol::ViewImageToolCallEvent;
use opencli_core::protocol::WarningEvent;
use opencli_core::protocol::WebSearchBeginEvent;
use opencli_core::protocol::WebSearchEndEvent;
use opencli_core::skills::model::SkillMetadata;
#[cfg(target_os = "windows")]
use opencli_core::windows_sandbox::WindowsSandboxLevelExt;
use opencli_otel::OtelManager;
use opencli_protocol::ThreadId;
use opencli_protocol::account::PlanType;
...
use opencli_protocol::models::local_image_label_text;
...
use chrono::Local;
use opencli_common::approval_presets::ApprovalPreset;
...
use opencli_file_search::FileMatch;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: opencli_feedback::OpenCLIFeedback,
...
fn rate_limit_error_kind(info: &OpenCLIErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        ...
        OpenCLIErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        OpenCLIErrorInfo::ResponseTooManyFailedAttempts {
...
/// and by sending requests back to opencli-core.
///
...
pub(crate) struct ChatWidget {
    ...
    opencli_op_tx: UnboundedSender<Op>,
    ...
    /// Tracks whether opencli-core currently considers an agent turn to be in progress.
    ///
    ...
    feedback: opencli_feedback::OpenCLIFeedback,
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: opencli_core::protocol::SessionConfiguredEvent) {
        self.bottom_pane
        ...
        // Ask opencli-core to enumerate custom prompts for this session.
    ...
    fn on_thread_name_updated(&mut self, event: opencli_core::protocol::ThreadNameUpdatedEvent) {
    ...
    fn on_patch_apply_end(&mut self, event: opencli_core::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_get_history_entry_response(
        &mut self,
        event: opencli_core::protocol::GetHistoryEntryResponseEvent,
    ) {
        let opencli_core::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: opencli_core::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        let opencli_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), thread_manager);
        ...
        widget.bottom_pane.set_windows_degraded_sandbox_active(
            opencli_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                && matches!(
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<opencli_core::OpenCLIThread>,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.otel_manager.counter(
                        "opencli.windows_sandbox.setup_elevated_sandbox_command",
                        1,
                        &[],
                    );
                    self.app_event_tx
            ...
            // SlashCommand::Undo => {
            //     self.app_event_tx.send(AppEvent::OpenCLIOp(Op::Undo));
            // }
            SlashCommand::Diff => {
            ...
            SlashCommand::TestApproval => {
                ...
                self.app_event_tx.send(AppEvent::OpenCLIEvent(Event {
                    id: "1".to_string(),
    ...
    fn dispatch_command_with_args(&mut self, cmd: SlashCommand, args: String) {
        ...
        match cmd {
            SlashCommand::Rename if !trimmed.is_empty() => {
                let Some(name) = opencli_core::util::normalize_thread_name(trimmed) else {
                ...
                self.request_redraw();
                self.app_event_tx
                    .send(AppEvent::OpenCLIOp(Op::SetThreadName { name }));
            }
    ...
    fn show_rename_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            ...
            Box::new(move |name: String| {
                ...
                tx.send(AppEvent::OpenCLIOp(Op::SetThreadName { name }));
            }),
        );
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        self.opencli_op_tx.send(op).unwrap_or_else(|e| {
    ...
    pub(crate) fn handle_opencli_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, false);
    }

    pub(crate) fn handle_opencli_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_opencli_event: {:?}", msg);
            }
        }
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                if let Some(info) = opencli_error_info
            ...
            EventMsg::ItemCompleted(event) => {
                if let opencli_protocol::items::TurnItem::Plan(plan_item) = event.item {
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, from_replay: bool) {
        ...
        let hint = review
            .user_facing_hint
            .unwrap_or_else(|| opencli_core::review_prompts::user_facing_hint(&review.target));
    ...
    fn prefetch_rate_limits(&mut self) {
        ...
        if !self
            ...
            .auth_cached()
            .as_ref()
            .is_some_and(OpenCLIAuth::is_chatgpt_auth)
        {
            return;
        }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::OpenCLIOp(Op::OverrideTurnContext {
                cwd: None,
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from(
            "Choose a communication style for OpenCLI. Disable in /experimental.".dim(),
        ));
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("opencli-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "opencli-auto-fast" => 0,
            "opencli-auto-balanced" => 1,
            "opencli-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running opencli -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    fn apply_model_and_effort(&self, model: String, effort: Option<ReasoningEffortConfig>) {
        self.app_event_tx
            .send(AppEvent::OpenCLIOp(Op::OverrideTurnContext {
                cwd: None,
                approval_policy: None,
                sandbox_policy: None,
                windows_sandbox_level: None,
    ...
    fn open_approval_mode_popup(&mut self, include_read_only: bool) {
        ...
        let show_elevate_sandbox_hint = opencli_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
        ...
        for preset in presets.into_iter() {
            ...
            let actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    {
                        let preset_clone = preset.clone();
                        if opencli_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && opencli_core::windows_sandbox::sandbox_setup_is_complete(
                                self.config.opencli_home.as_path(),
                            )
                        {
                            vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match opencli_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.opencli_home.as_path(),
            cwd.as_path(),
            &env_map,
            self.config.sandbox_policy.get(),
            Some(self.config.opencli_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When OpenCLI runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        self.otel_manager
            .counter("opencli.windows_sandbox.elevated_prompt_shown", 1, &[]);
        ...
        stay_actions.insert(
            ...
            Box::new({
                ...
                move |_tx| {
                    otel.counter("opencli.windows_sandbox.elevated_prompt_decline", 1, &[]);
                }
            }),
        );
        ...
        let items = vec![
            SelectionItem {
                ...
                description: None,
                actions: vec![Box::new(move |tx| {
                    accept_otel.counter("opencli.windows_sandbox.elevated_prompt_accept", 1, &[]);
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        stay_actions.insert(
            ...
            Box::new({
                let otel = self.otel_manager.clone();
                move |_tx| {
                    otel.counter("opencli.windows_sandbox.fallback_stay_current", 1, &[]);
                }
            }),
        );
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter("opencli.windows_sandbox.fallback_retry_elevated", 1, &[]);
                        tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
                            preset: preset.clone(),
                        });
                    }
            ...
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter("opencli.windows_sandbox.fallback_use_legacy", 1, &[]);
                        tx.send(AppEvent::EnableWindowsSandboxForAgentMode {
                            preset: preset.clone(),
                            mode: WindowsSandboxEnableMode::Legacy,
                        });
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = opencli_core::util::resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("opencli resume {name}"));
        ...
        for connector in connectors {
            ...
            let (selected_label, missing_label, instructions) = if connector.is_accessible {
                ...
                (
                    ...
                    "Install this app in your browser, then reload OpenCLI.",
                )
            };
    ...
    /// Forward an `Op` directly to opencli.
    pub(crate) fn submit_op(&mut self, op: Op) {
        ...
        if let Err(e) = self.opencli_op_tx.send(op) {
    ...
    pub(crate) fn open_review_popup(&mut self) {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx: &AppEventSender| {
                tx.send(AppEvent::OpenCLIOp(Op::Review {
                    review_request: ReviewRequest {
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::OpenCLIOp(Op::Review {
                        review_request: ReviewRequest {
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        let commits = opencli_core::git_info::recent_commits(cwd, 100).await;
...
async fn fetch_rate_limits(base_url: String, auth: OpenCLIAuth) -> Option<RateLimitSnapshot> {
...
pub(crate) fn show_review_commit_picker_with_entries(
    chat: &mut ChatWidget,
    entries: Vec<opencli_core::git_info::CommitLogEntry>,
) {

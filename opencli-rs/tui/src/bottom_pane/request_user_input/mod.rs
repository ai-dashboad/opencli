...
impl RequestUserInputOverlay {
    ...
    fn current_question(
        &self,
    ) -> Option<&opencli_protocol::request_user_input::RequestUserInputQuestion> {
    ...
    fn options_len_for_question(
        question: &opencli_protocol::request_user_input::RequestUserInputQuestion,
    ) -> usize {
    ...
    fn submit_answers(&mut self) {
        ...
        self.app_event_tx
            .send(AppEvent::OpenCLIOp(Op::UserInputAnswer {
...
mod tests {
    ...
    fn options_can_submit_empty_when_unanswered() {
        ...
        let AppEvent::OpenCLIOp(Op::UserInputAnswer { id, response }) = event else {
    ...
    fn enter_commits_default_selection_on_last_option_question() {
        ...
        let event = rx.try_recv().expect("expected AppEvent");
        let AppEvent::OpenCLIOp(Op::UserInputAnswer { response, .. }) = event else {
            panic!("expected UserInputAnswer");
        };
        let answer = response.answers.get("q1").expect("answer missing");
    ...
    fn esc_in_notes_mode_without_options_interrupts() {
        ...
        let event = rx.try_recv().expect("expected AppEvent");
        let AppEvent::OpenCLIOp(op) = event else {
            panic!("expected OpenCLIOp");
        };

...
impl BottomPane {
    ...
    /// Update the status indicator header (defaults to "Processing") and details below it.
    ///
...
mod tests {
    ...
    fn composer_shown_after_denied_while_task_running() {
        ...
        assert!(
            row0.contains("Processing"),
            "expected Processing header after denial on row 0: {row0:?}"
        );
    ...
    fn esc_with_skill_popup_does_not_interrupt_task() {
        ...
        while let Ok(ev) = rx.try_recv() {
            assert!(
                !matches!(ev, AppEvent::OpenCLIOp(Op::Interrupt)),
    ...
    fn esc_interrupts_running_task_when_no_popup() {
        ...
        assert!(
            matches!(rx.try_recv(), Ok(AppEvent::OpenCLIOp(Op::Interrupt))),

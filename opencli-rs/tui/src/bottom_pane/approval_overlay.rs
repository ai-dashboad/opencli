...
impl ApprovalOverlay {
    ...
    fn handle_elicitation_decision(
        ...
        self.app_event_tx
            .send(AppEvent::OpenCLIOp(Op::ResolveElicitation {
...
fn exec_options(
    ...
    .chain([ApprovalOption {
        label: "No, and tell OpenCLI what to do differently".to_string(),
...
mod tests {
    ...
    fn shortcut_triggers_selection() {
        ...
        // We expect at least one OpenCLIOp message in the queue.
        let mut saw_op = false;
        while let Ok(ev) = rx.try_recv() {
            if matches!(ev, AppEvent::OpenCLIOp(_)) {
                saw_op = true;
                break;
            }
        }
    ...
    fn exec_prefix_option_emits_execpolicy_amendment() {
        ...
        let mut saw_op = false;
        while let Ok(ev) = rx.try_recv() {
            if let AppEvent::OpenCLIOp(Op::ExecApproval { decision, .. }) = ev {
                assert_eq!(
                    decision,
                    ReviewDecision::ApprovedExecpolicyAmendment {
    ...
    fn enter_sets_last_selected_index_without_dismissing() {
        ...
        while let Ok(ev) = rx.try_recv() {
            if let AppEvent::OpenCLIOp(Op::ExecApproval { decision: d, .. }) = ev {
                decision = Some(d);
                break;
            }
        }

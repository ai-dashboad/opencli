...
// The standalone `opencli-tui` binary prints a short help message before the
...
use opencli_core::config::load_config_as_toml_with_cli_overrides;
use opencli_core::config::resolve_oss_provider;
use opencli_core::config_loader::CloudRequirementsLoader;
use opencli_core::config_loader::ConfigLoadError;
use opencli_core::config_loader::format_config_error_with_source;
use opencli_core::default_client::set_default_client_residency_requirement;
use opencli_core::find_thread_path_by_id_str;
use opencli_core::find_thread_path_by_name_str;
use opencli_core::path_utils;
use opencli_core::protocol::AskForApproval;
use opencli_core::read_session_meta_line;
use opencli_core::terminal::Multiplexer;
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = opencli_common::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let opencli_home = match find_opencli_home() {
        Ok(opencli_home) => opencli_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding opencli home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if let Err(err) =
        opencli_core::personality_migration::maybe_migrate_personality(&opencli_home, &config_toml)
            .await
    {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&opencli_home).await?;
    ...
    let log_dir = opencli_core::config::log_dir(&config)?;
    ...
    // OpenCLI CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("opencli-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for opencli crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("opencli_core=info,opencli_tui=info,opencli_rmcp_client=info")
        })
    };
    ...
    let feedback = opencli_feedback::OpenCLIFeedback::new();
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        opencli_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, true)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
            #[allow(clippy::print_stderr)]
    ...
    let log_db_layer = opencli_core::state_db::get_state_db(&config, None)
        .await
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: opencli_core::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let auth_manager = AuthManager::shared(
        initial_config.opencli_home.clone(),
        false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `opencli {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            let path = if is_uuid {
                find_thread_path_by_id_str(&config.opencli_home, id_str).await?
            } else {
                find_thread_path_by_name_str(&config.opencli_home, id_str).await?
            };
            match path {
...
mod tests {
    ...
    use opencli_core::config::ConfigBuilder;
    use opencli_core::config::ConfigOverrides;
    use opencli_core::config::ProjectConfig;
    use opencli_core::protocol::AskForApproval;
    use opencli_protocol::protocol::RolloutItem;
    use opencli_protocol::protocol::RolloutLine;
    use opencli_protocol::protocol::SessionMeta;
    ...
    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .opencli_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let opencli_home = temp_dir.path().to_path_buf();

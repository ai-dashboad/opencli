...
pub use opencli_app_server_protocol::AppInfo;
...
pub async fn list_accessible_connectors_from_mcp_tools(
    ...
    let mcp_servers = with_opencli_apps_mcp(HashMap::new(), true, auth.as_ref(), config);
    ...
    if let Some(cfg) = mcp_servers.get(OPENCLI_APPS_MCP_SERVER_NAME) {
        let timeout = cfg.startup_timeout_sec.unwrap_or(DEFAULT_STARTUP_TIMEOUT);
        mcp_connection_manager
            .wait_for_server_ready(OPENCLI_APPS_MCP_SERVER_NAME, timeout)
            .await;
    }

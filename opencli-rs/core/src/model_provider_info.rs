//! Registry of model providers supported by OpenCLI.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so OpenCLI works out-of-the-box.
//!   2. User-defined entries inside `~/.opencli/config.toml` under the `model_providers`
...
const DEFAULT_STREAM_MAX_RETRIES: u64 = 8;
...
impl ModelProviderInfo {
    ...
    pub(crate) fn to_api_provider(
        ...
        let default_base_url = if matches!(auth_mode, Some(AuthMode::Chatgpt)) {
            "https://chatgpt.com/backend-api/opencli"
        } else {
        ...
        match &self.env_key {
            Some(env_key) => {
                ...
                env_value
                    ...
                    .map_err(|_| {
                        crate::error::OpenCLIErr::EnvVar(EnvVarError {
                            var: env_key.clone(),
    ...
    pub fn create_openai_provider() -> ModelProviderInfo {
        ModelProviderInfo {
            ...
            // OpenCLI at a proxy, mock server, or Azure-style deployment
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These OPENCLI_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let opencli_oss_base_url = match std::env::var("OPENCLI_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
    {
        Some(url) => url,
        None => format!(
            ...
            port = std::env::var("OPENCLI_OSS_PORT")
                .ok()
                .filter(|v| !v.trim().is_empty())
    ...
    create_oss_provider_with_base_url(&opencli_oss_base_url, wire_api)
}

...
use opencli_state::DB_METRIC_COMPARE_ERROR;
pub use opencli_state::LogEntry;
use opencli_state::STATE_DB_FILENAME;
use opencli_state::ThreadMetadataBuilder;
...
pub type StateDbHandle = Arc<opencli_state::StateRuntime>;
...
pub(crate) async fn init_if_enabled(
    ...
    let state_path = config.opencli_home.join(STATE_DB_FILENAME);
    if !config.features.enabled(Feature::Sqlite) {
        return None;
    }
    let existed = tokio::fs::try_exists(&state_path).await.unwrap_or(false);
    let runtime = match opencli_state::StateRuntime::init(
    ...
    .await
    {
        ...
        Err(err) => {
            warn!(
                ...
                config.opencli_home.display()
            );
            if let Some(otel) = otel {
                otel.counter("opencli.db.init", 1, &[("status", "init_error")]);
            }
            return None;
        }
    };
...
pub async fn get_state_db(config: &Config, otel: Option<&OtelManager>) -> Option<StateDbHandle> {
    ...
    opencli_state::StateRuntime::init(
        config.opencli_home.clone(),
        config.model_provider_id.clone(),
        otel.cloned(),
    )
...
pub async fn open_if_present(opencli_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = opencli_home.join(STATE_DB_FILENAME);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime = opencli_state::StateRuntime::init(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<opencli_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    ...
    let ctx = context?;
    if ctx.opencli_home() != opencli_home {
        warn!(
            "state db opencli_home mismatch: expected {}, got {}",
            ctx.opencli_home().display(),
            opencli_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => opencli_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => opencli_state::SortKey::UpdatedAt,
            },
...
pub fn record_discrepancy(stage: &str, reason: &str) {
    ...
    if let Some(metric) = opencli_otel::metrics::global() {

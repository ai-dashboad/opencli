...
use opencli_app_server_protocol::AuthMode as ApiAuthMode;
...
use opencli_client::OpenCLIHttpClient;
...
struct ChatgptAuthState {
    ...
    client: OpenCLIHttpClient,
}

impl PartialEq for OpenCLIAuth {
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "OPENCLI_REFRESH_TOKEN_URL_OVERRIDE";
...
impl OpenCLIAuth {
    fn from_auth_dot_json(
        ...
        if auth_mode == ApiAuthMode::ApiKey {
            ...
            return Ok(OpenCLIAuth::from_api_key_with_client(api_key, client));
        }
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(opencli_home.to_path_buf(), storage_mode);
    ...
    pub fn from_auth_storage(
        ...
        load_auth(opencli_home, false, auth_credentials_store_mode)
    }
    ...
    fn from_api_key_with_client(api_key: &str, _client: OpenCLIHttpClient) -> Self {
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &OpenCLIHttpClient {
        &self.state.client
    }
}

pub const OPENAI_API_KEY_ENV_VAR: &str = "OPENAI_API_KEY";
pub const OPENCLI_API_KEY_ENV_VAR: &str = "OPENCLI_API_KEY";
...
pub fn read_opencli_api_key_from_env() -> Option<String> {
    env::var(OPENCLI_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `opencli_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    opencli_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(opencli_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(opencli_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(opencli_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(opencli_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(opencli_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(opencli_home, auth_credentials_store_mode)?;
    Ok(removed_ephemeral || removed_managed)
}

fn load_auth(
    opencli_home: &Path,
    enable_opencli_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<OpenCLIAuth>> {
    let build_auth = |auth_dot_json: AuthDotJson, storage_mode| {
        let client = crate::default_client::create_client();
        OpenCLIAuth::from_auth_dot_json(opencli_home, auth_dot_json, storage_mode, client)
    };

    // API key via env var takes precedence over any other auth method.
    if enable_opencli_api_key_env && let Some(api_key) = read_opencli_api_key_from_env() {
        let client = crate::default_client::create_client();
        return Ok(Some(OpenCLIAuth::from_api_key_with_client(
            api_key.as_str(),
            client,
        )));
    }
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(OpenCLIAuth::api_auth_mode),
            )
            .field(
                "external_refresher",
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(OpenCLIAuth::get_account_id);
        let mode = if cached_auth
            .as_ref()
            .is_some_and(OpenCLIAuth::is_external_chatgpt_tokens)
        {
...
/// hands out cloned `OpenCLIAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific OpenCLIAuth, for testing only.
    pub fn from_auth_for_testing(auth: OpenCLIAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            opencli_home: PathBuf::from("non-existent"),
            inner: RwLock::new(cached),
            enable_opencli_api_key_env: false,
    ...
    #[cfg(any(test, feature = "test-support"))]
    /// Create an AuthManager with a specific OpenCLIAuth and opencli home, for testing only.
    pub fn from_auth_for_testing_with_home(auth: OpenCLIAuth, opencli_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth {
            auth: Some(auth),
            external_refresher: None,
        };
    ...
    fn reload_if_account_id_matches(&self, expected_account_id: Option<&str>) -> ReloadOutcome {
        ...
        let new_auth = self.load_auth_from_storage();
        let new_account_id = new_auth.as_ref().and_then(OpenCLIAuth::get_account_id);
    ...
    fn auths_equal(a: Option<&OpenCLIAuth>, b: Option<&OpenCLIAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    fn load_auth_from_storage(&self) -> Option<OpenCLIAuth> {
        load_auth(
            &self.opencli_home,
            self.enable_opencli_api_key_env,
    ...
    fn set_cached_auth(&self, new_auth: Option<OpenCLIAuth>) -> bool {
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        match auth {
            OpenCLIAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            OpenCLIAuth::Chatgpt(chatgpt_auth) => {
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.opencli_home, self.auth_credentials_store_mode)?;
    ...
    pub fn get_auth_mode(&self) -> Option<ApiAuthMode> {
        self.auth_cached().as_ref().map(OpenCLIAuth::api_auth_mode)
    }

    pub fn get_internal_auth_mode(&self) -> Option<AuthMode> {
        self.auth_cached()
            .as_ref()
            .map(OpenCLIAuth::internal_auth_mode)
    }

    async fn refresh_if_stale(&self, auth: &OpenCLIAuth) -> Result<bool, RefreshTokenError> {
        let chatgpt_auth = match auth {
            OpenCLIAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return Ok(false),
        };
    ...
    async fn refresh_external_auth(
        ...
        let previous_account_id = self
            .auth_cached()
            .as_ref()
            .and_then(OpenCLIAuth::get_account_id);
...
mod tests {
    ...
    fn missing_auth_json_returns_none() {
        let dir = tempdir().unwrap();
        let auth = OpenCLIAuth::from_auth_storage(dir.path(), AuthCredentialsStoreMode::File)
            .expect("call should succeed");
        assert_eq!(auth, None);
    }

    #[tokio::test]
    #[serial(opencli_api_key)]
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let auth = super::load_auth(opencli_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, opencli_home: &Path) -> std::io::Result<String> {
        let auth_file = get_auth_file(opencli_home);
    ...
    async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
        ...
        assert!(
            !opencli_home.path().join("auth.json").exists(),

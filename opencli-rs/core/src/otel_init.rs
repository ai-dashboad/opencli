...
use opencli_otel::config::OtelTlsConfig as OtelTlsSettings;
...
pub fn build_provider(
    ...
    // opencli: force every OTEL exporter to None so metrics/traces are never
    // exported over the network, regardless of configuration.
    let to_otel_exporter = |_kind: &Kind| OtelExporter::None;
    #[allow(unreachable_code, unused)]
    let _unused_to_otel_exporter = |kind: &Kind| match kind {
...
/// Filter predicate for exporting only OpenCLI-owned events via OTEL.
/// Keeps events that originated from opencli_otel module
pub fn opencli_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("opencli_otel")
}

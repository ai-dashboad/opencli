...
use opencli_app_server_protocol::Tools;
use opencli_app_server_protocol::UserSavedConfig;
use opencli_protocol::config_types::AltScreenMode;
...
pub use opencli_git::GhostSnapshotConfig;
...
pub(crate) fn test_config() -> Config {
    let opencli_home = tempdir().expect("create temp dir");
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, OpenCLI will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - OpenCLI
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.opencli/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "OpenCLI"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send OpenCLI '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the OpenCLI home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that OpenCLI can reach out to for tool calls.
    ...
    ///
    /// When unset, OpenCLI will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all OpenCLI state (defaults to `~/.opencli` but can be
    /// overridden by the `OPENCLI_HOME` environment variable).
    pub opencli_home: PathBuf,
    ...
    /// Path to the `opencli-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `opencli-linux-sandbox`.
    pub opencli_linux_sandbox_exe: Option<PathBuf>,
    ...
    /// When `true`, checks for OpenCLI updates on startup and surfaces update prompts.
    /// Set to `false` only if your OpenCLI updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across OpenCLI product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across OpenCLI product surfaces.
    /// Defaults to `true`.
...
impl ConfigBuilder {
    pub fn opencli_home(mut self, opencli_home: PathBuf) -> Self {
        self.opencli_home = Some(opencli_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let opencli_home = opencli_home.map_or_else(find_opencli_home, std::io::Result::Ok)?;
...
impl Config {
    ...
    pub fn load_default_with_cli_overrides(
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &opencli_home)?;
    ...
    /// ignores user settings. For example, the `opencli exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd` and `opencli_linux_sandbox_exe`.
...
pub async fn load_config_as_toml_with_cli_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, opencli_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .opencli/ folders.
...
/// Patch `OPENCLI_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(opencli_home: &Path, provider: &str) -> std::io::Result<()> {
...
/// Base config deserialized from ~/.opencli/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by OpenCLI will likely degrade model performance.
    ...
    /// Per-model token pricing for the `/status` cost estimate. See [`ModelPricing`].
    #[serde(default)]
    ...
    /// directories for `.opencli` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
impl Config {
    ...
    fn load_from_base_config_with_overrides(
        ...
        Self::load_config_with_layer_stack(cfg, overrides, opencli_home, config_layer_stack)
    }
    ...
    fn load_config_with_layer_stack(
        ...
        let user_instructions = Self::load_instructions(Some(&opencli_home));
    ...
    fn load_instructions(opencli_dir: Option<&Path>) -> Option<String> {
        let base = opencli_dir?;
...
/// Returns the path to the OpenCLI configuration directory, which can be
/// specified by the `OPENCLI_HOME` environment variable. If not set, defaults to
/// `~/.opencli`.
///
/// - If `OPENCLI_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `OPENCLI_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_opencli_home() -> std::io::Result<PathBuf> {
...
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.opencli_home.clone();
    p.push("log");
    Ok(p)
}
...
"#;
    ...
    async fn project_profile_overrides_user_profile() -> std::io::Result<()> {
        ...
        std::fs::write(
            opencli_home.path().join(CONFIG_TOML_FILE),
            format!(
                r#"
profile = "global"
...
"#,
        ...
        let project_config_dir = workspace.path().join(".opencli");
...
"#,
    ...
    async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
        let opencli_home = TempDir::new()?;
        let managed_path = opencli_home.path().join("managed_config.toml");
        let config_path = opencli_home.path().join(CONFIG_TOML_FILE);
        ...
        let cwd = AbsolutePathBuf::try_from(opencli_home.path())?;
        let config_layer_stack = load_config_layers_state(
            opencli_home.path(),
            Some(cwd),
            &Vec::new(),
    ...
    async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        ...
        let servers = load_global_mcp_servers(opencli_home.path()).await?;
    ...
    async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        let loaded = load_global_mcp_servers(opencli_home.path()).await?;
...
"#,
        ...
        let err = load_global_mcp_servers(opencli_home.path())
            .await
...
"#
    ...
    async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
        ...
        let cwd_path = PathBuf::from("/tmp/opencli-mcp");
        let servers = BTreeMap::from([(
            "docs".to_string(),
            McpServerConfig {
                transport: McpServerTransportConfig::Stdio {
        ...
        let serialized = std::fs::read_to_string(&config_path)?;
        assert!(
            serialized.contains(r#"cwd = "/tmp/opencli-mcp""#),
        ...
        match &docs.transport {
            McpServerTransportConfig::Stdio { cwd, .. } => {
                assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/opencli-mcp")));
            }
            other => panic!("unexpected transport {other:?}"),
        }
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn opencli_home(&self) -> PathBuf {
            self.opencli_home.path().to_path_buf()
        }
    }
    ...
    fn loads_compact_prompt_from_file() -> std::io::Result<()> {
        let opencli_home = TempDir::new()?;
        let workspace = opencli_home.path().join("workspace");
        std::fs::create_dir_all(&workspace)?;
...
"#;
        ...
        let opencli_home_temp_dir = TempDir::new().unwrap();
        ...
        Ok(PrecedenceTestFixture {
            cwd: cwd_temp_dir,
            opencli_home: opencli_home_temp_dir,
            cfg,
            model_provider_map,
            openai_provider,
...
projects = { "/Users/mbolin/code/opencli4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/opencli3" = { trust_level = "trusted" } }
model = "foo""#;
        let mut doc = initial.parse::<DocumentMut>()?;

        // Approve a new directory
        let new_project = Path::new("/Users/mbolin/code/opencli2");
...
[projects."/Users/mbolin/code/opencli4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/opencli3"]
trust_level = "trusted"

[projects."/Users/mbolin/code/opencli2"]
trust_level = "trusted"
"#;
        assert_eq!(contents, expected);
    ...
    fn test_set_default_oss_provider() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let opencli_home = temp_dir.path();
        let config_path = opencli_home.join(CONFIG_TOML_FILE);

        // Test setting valid provider on empty config
        set_default_oss_provider(opencli_home, OLLAMA_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        assert!(content.contains("oss_provider = \"ollama\""));

        // Test updating existing config
        std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
        set_default_oss_provider(opencli_home, LMSTUDIO_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        ...
        let result = set_default_oss_provider(opencli_home, "invalid_provider");

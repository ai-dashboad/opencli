...
pub async fn apply(
    ...
    task::spawn_blocking(move || apply_blocking(&opencli_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(opencli_home: &Path) -> Self {
        Self {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.opencli_home, self.profile.as_deref(), &self.edits)
    }
...
"#,
    ...
    fn blocking_set_model_writes_through_symlink_chain() {
        ...
        let link_path = opencli_home.join("config-link.toml");
        let config_path = opencli_home.join(CONFIG_TOML_FILE);
...
"#;
    ...
    fn blocking_set_model_replaces_symlink_on_cycle() {
        let tmp = tempdir().expect("tmpdir");
        let opencli_home = tmp.path();
        let link_a = opencli_home.join("a.toml");
        let link_b = opencli_home.join("b.toml");
        let config_path = opencli_home.join(CONFIG_TOML_FILE);
...
"#;
        std::fs::write(opencli_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
"#;
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !opencli_home.join(CONFIG_TOML_FILE).exists(),

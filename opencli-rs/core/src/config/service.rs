...
use opencli_app_server_protocol::Config as ApiConfig;
use opencli_app_server_protocol::ConfigBatchWriteParams;
use opencli_app_server_protocol::ConfigLayerMetadata;
...
use opencli_app_server_protocol::OverriddenMetadata;
...
impl ConfigService {
    ...
    pub fn new_with_defaults(opencli_home: PathBuf) -> Self {
        Self {
            opencli_home,
        ...
        let layers = match params.cwd.as_deref() {
            Some(cwd) => {
                ...
                crate::config::ConfigBuilder::default()
                    .opencli_home(self.opencli_home.clone())
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<opencli_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.opencli_home)
        ...
        if !config_edits.is_empty() {
            ConfigEditsBuilder::new(&self.opencli_home)
    ...
    /// include any in-repo .opencli/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_opencli_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_opencli_folder.display(),
        ),
...
"#;
    ...
    async fn write_value_preserves_comments_and_order() -> Result<()> {
        ...
        let original = r#"# OpenCLI user configuration
model = "gpt-5"
...
"#;
        ...
        let expected = r#"# OpenCLI user configuration
model = "gpt-5"
approval_policy = "on-request"

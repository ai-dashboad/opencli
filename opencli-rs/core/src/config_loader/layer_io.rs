...
#[cfg(unix)]
const OPENCLI_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/opencli/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/opencli/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(opencli_home)),
    )?;
...
pub(super) fn managed_config_default_path(opencli_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = opencli_home;
        PathBuf::from(OPENCLI_MANAGED_CONFIG_SYSTEM_PATH)
    }

    #[cfg(not(unix))]
    {
        opencli_home.join("managed_config.toml")
    }
}

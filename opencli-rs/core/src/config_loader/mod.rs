...
const DEFAULT_REQUIREMENTS_TOML_FILE_UNIX: &str = "/etc/opencli/requirements.toml";

/// On Unix systems, load default settings from this file path, if present.
/// Note that /etc/opencli/ is treated as a "config folder," so subfolders such
/// as skills/ and rules/ will also be honored.
pub const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/opencli/config.toml";
...
/// - system    `/etc/opencli/requirements.toml`
///
/// For backwards compatibility, we also load from
/// `/etc/opencli/managed_config.toml` and map it to
/// `/etc/opencli/requirements.toml`.
///
/// Configuration is built up from multiple layers in the following order:
///
/// - admin:    managed preferences (*)
/// - system    `/etc/opencli/config.toml`
/// - user      `${OPENCLI_HOME}/config.toml`
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.opencli/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.opencli/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://github.com/ai-dashboad/opencli/blob/main/docs/sandbox.md for details.
///
...
pub async fn load_config_layers_state(
    ...
    // Honor /etc/opencli/requirements.toml.
    if cfg!(unix) {
    ...
    // Add a layer for $OPENCLI_HOME/config.toml if it exists. Note if the file
    // exists, but is malformed, then this error should be propagated to the
    // user.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, opencli_home)?;
...
/// If available, apply requirements from `/etc/opencli/requirements.toml` to
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_opencli_folder: dot_opencli_folder.clone(),
    };
...
async fn load_project_layers(
    ...
    for dir in dirs {
        let dot_opencli = dir.join(".opencli");
        if !tokio::fs::metadata(&dot_opencli)
            .await
        ...
        match tokio::fs::read_to_string(&config_file).await {
            Ok(contents) => {
                ...
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_opencli_abs.as_path())?;
                let entry =
                    project_layer_entry(trust_context, &dot_opencli_abs, &layer_dir, config, true);
                layers.push(entry);
            }
            Err(err) => {
...
/// from a file like `/etc/opencli/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for OpenCLI to function correctly.

...
/// Base config deserialized from /etc/opencli/requirements.toml or MDM.
...
mod tests {
    ...
    use opencli_protocol::protocol::NetworkAccess;
    use opencli_utils_absolute_path::AbsolutePathBuf;
    ...
    fn constraint_error_includes_requirement_source() -> Result<()> {
        ...
        let requirements_toml_file = if cfg!(windows) {
            "C:\\etc\\opencli\\requirements.toml"
        } else {
    ...
    fn exec_policy_error_includes_requirement_source() -> Result<()> {
        ...
        let requirements_toml_file =
            AbsolutePathBuf::from_absolute_path("/etc/opencli/requirements.toml")?;
        let source_location = RequirementSource::SystemRequirementsToml {
            file: requirements_toml_file,
        };

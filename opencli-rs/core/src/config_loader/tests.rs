...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .opencli_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = opencli_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when OPENCLI_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let opencli_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&opencli_home).await?;
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".opencli")).await?;
    tokio::fs::create_dir_all(project_root.join(".opencli")).await?;
    ...
    .await?;
    tokio::fs::write(
        nested.join(".opencli").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let opencli_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&opencli_home).await?;
    make_config_for_test(&opencli_home, &project_root, TrustLevel::Trusted, None).await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            super::ConfigLayerSource::Project { dot_opencli_folder } => Some(dot_opencli_folder),
            _ => None,
        })
        .collect();
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_opencli_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(
        nested.join(".opencli").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let opencli_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&opencli_home).await?;
    tokio::fs::write(opencli_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn opencli_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let opencli_home = home_dir.join(".opencli");
    tokio::fs::create_dir_all(&opencli_home).await?;
    tokio::fs::write(opencli_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn opencli_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_opencli = project_root.join(".opencli");
    let nested_dot_opencli = nested.join(".opencli");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_opencli_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_opencli)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let opencli_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&opencli_home_untrusted).await?;
    make_config_for_test(
        &opencli_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        None,
    )
    .await?;
    let untrusted_config_path = opencli_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let opencli_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&opencli_home_unknown).await?;
    tokio::fs::write(
        opencli_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;

    let layers_unknown = load_config_layers_state(
        &opencli_home_unknown,
        Some(cwd),
        &[] as &[(String, TomlValue)],
        LoaderOverrides::default(),
        CloudRequirementsLoader::default(),
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".opencli")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".opencli").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let opencli_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&opencli_home).await?;
        let config_path = opencli_home.join(CONFIG_TOML_FILE);

        if let Some(trust_level) = trust_level {
            make_config_for_test(&opencli_home, &project_root, trust_level, None).await?;
...
mod requirements_exec_policy_tests {
    ...
    use opencli_execpolicy::Evaluation;
    use opencli_execpolicy::RuleMatch;
    use opencli_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_opencli_folder_with_requirements(
        dot_opencli_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_opencli_folder = AbsolutePathBuf::from_absolute_path(dot_opencli_folder)
            .expect("absolute dot_opencli_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_opencli_folder },
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_opencli_folder_with_requirements(temp_dir.path(), requirements);

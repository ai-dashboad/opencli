...
use opencli_app_server_protocol::ConfigLayer;
...
impl ConfigLayerEntry {
    ...
    // Get the `.opencli/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ConfigLayerSource::Mdm { .. } => None,
            ConfigLayerSource::System { file } => file.parent(),
            ConfigLayerSource::User { file } => file.parent(),
            ConfigLayerSource::Project { dot_opencli_folder } => Some(dot_opencli_folder.clone()),
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_opencli_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_opencli_folder: current_project_dot_opencli_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_opencli_folder {
                ...
                if previous == current_project_dot_opencli_folder
                    || !current_project_dot_opencli_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_opencli_folder = Some(current_project_dot_opencli_folder);
        }
    }

...
use crate::spawn::OPENCLI_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use crate::tools::sandboxing::SandboxablePreference;
use opencli_protocol::config_types::WindowsSandboxLevel;
pub use opencli_protocol::models::SandboxPermissions;
...
pub(crate) enum SandboxTransformError {
    #[error("missing opencli-linux-sandbox executable path")]
...
impl SandboxManager {
    ...
    pub(crate) fn transform(
        ...
        if !policy.has_full_network_access() {
            env.insert(
                OPENCLI_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        ...
        let (command, sandbox_env, arg0_override) = match sandbox {
            ...
            SandboxType::MacosSeatbelt => {
                ...
                seatbelt_env.insert(OPENCLI_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
                let mut args =
            ...
            SandboxType::LinuxSeccomp => {
                let exe = opencli_linux_sandbox_exe
                ...
                (
                    full_command,
                    HashMap::new(),
                    Some("opencli-linux-sandbox".to_string()),
                )
            }
            // On Windows, the restricted token sandbox executes in-process via the
            // opencli-windows-sandbox crate. We leave the command unchanged here and

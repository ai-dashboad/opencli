...
use opencli_state::BackfillStats;
use opencli_state::DB_ERROR_METRIC;
use opencli_state::DB_METRIC_BACKFILL;
use opencli_state::DB_METRIC_BACKFILL_DURATION_MS;
use opencli_state::ExtractionOutcome;
use opencli_state::ThreadMetadataBuilder;
use opencli_state::apply_rollout_item;
use std::cmp::Reverse;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(
    runtime: &opencli_state::StateRuntime,
    config: &Config,
    otel: Option<&OtelManager>,
) {
    let timer = otel.and_then(|otel| otel.start_timer(DB_METRIC_BACKFILL_DURATION_MS, &[]).ok());
    let sessions_root = config.opencli_home.join(rollout::SESSIONS_SUBDIR);
    let archived_root = config.opencli_home.join(rollout::ARCHIVED_SESSIONS_SUBDIR);
...
mod tests {
    ...
    use chrono::Utc;
    use opencli_protocol::ThreadId;
    use opencli_protocol::protocol::CompactedItem;
    use opencli_protocol::protocol::RolloutItem;

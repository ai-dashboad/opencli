...
use crate::spawn::OPENCLI_SANDBOX_ENV_VAR;
use opencli_client::OpenCLIHttpClient;
pub use opencli_client::OpenCLIRequestBuilder;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(OPENCLI_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(OPENCLI_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "opencli_vscode"
        || originator_value.starts_with("OpenCLI ")
}

pub fn get_opencli_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized OpenCLI user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base OpenCLI user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default OpenCLI originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> OpenCLIHttpClient {
    let inner = build_reqwest_client();
    OpenCLIHttpClient::new(inner)
}
...
fn is_sandboxed() -> bool {
    std::env::var(OPENCLI_SANDBOX_ENV_VAR).as_deref() == Ok("seatbelt")
}
...
mod tests {
    ...
    #[test]
    fn test_get_opencli_user_agent() {
    ...
    fn is_first_party_originator_matches_known_values() {
        ...
        assert_eq!(is_first_party_originator("opencli_vscode"), true);
        assert_eq!(is_first_party_originator("OpenCLI Something Else"), true);
        assert_eq!(is_first_party_originator("opencli_cli"), false);
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        // User-Agent matches the computed OpenCLI UA for that originator
        let expected_ua = get_opencli_user_agent();
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "opencli_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "opencli_cli_rs/0.0.0 (bad_suffix)"
        );
    }

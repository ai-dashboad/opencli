...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, OpenCLIErr>> {
        ...
        async move {
            match handle.await {
                ...
                Ok(Err(FunctionCallError::Fatal(message))) => Err(OpenCLIErr::Fatal(message)),
                Ok(Err(other)) => Err(OpenCLIErr::Fatal(other.to_string())),
                Err(err) => Err(OpenCLIErr::Fatal(format!(
...
impl ToolCallRuntime {
    ...
    fn abort_message(call: &ToolCall, secs: f32) -> String {
        match call.tool_name.as_str() {
            "run" | "container_run" | "local_shell" | "run_command" | "unified_exec" => {

...
//! `opencli --opencli-run-as-apply-patch`, and runs under the current
//! `SandboxAttempt` with a minimal environment.
use crate::OPENCLI_APPLY_PATCH_ARG1;
...
pub struct ApplyPatchRequest {
    ...
    pub opencli_exe: Option<PathBuf>,
}
...
impl ApplyPatchRuntime {
    ...
    fn build_command_spec(req: &ApplyPatchRequest) -> Result<CommandSpec, ToolError> {
        use std::env;
        let exe = if let Some(path) = &req.opencli_exe {
            path.clone()
        } else {
            env::current_exe()
                .map_err(|e| ToolError::Rejected(format!("failed to determine opencli exe: {e}")))?
        };
...
impl ToolRuntime<ApplyPatchRequest, ExecToolCallOutput> for ApplyPatchRuntime {
    async fn run(
        ...
        let env = attempt
            .env_for(spec)
            .map_err(|err| ToolError::OpenCLI(err.into()))?;
        let out = execute_env(env, attempt.policy, Self::stdout_stream(ctx))
            .await
            .map_err(ToolError::OpenCLI)?;
        Ok(out)
    }
}

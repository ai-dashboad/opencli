...
/// Represents a newly created OpenCLI thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<OpenCLIThread>,
...
pub struct ThreadManager {
    ...
    #[cfg(any(test, feature = "test-support"))]
    _test_opencli_home_guard: Option<TempDir>,
}
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<OpenCLIThread>>>>,
...
impl ThreadManager {
    pub fn new(
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                skills_manager: Arc::new(SkillsManager::new(opencli_home)),
                auth_manager,
                session_source,
                #[cfg(any(test, feature = "test-support"))]
            ...
            #[cfg(any(test, feature = "test-support"))]
            _test_opencli_home_guard: None,
        }
    }

    #[cfg(any(test, feature = "test-support"))]
    /// Construct with a dummy AuthManager containing the provided OpenCLIAuth.
    /// Used for integration tests: should not be used by ordinary business logic.
    pub fn with_models_provider(auth: OpenCLIAuth, provider: ModelProviderInfo) -> Self {
        let temp_dir = tempfile::tempdir().unwrap_or_else(|err| panic!("temp opencli home: {err}"));
        let opencli_home = temp_dir.path().to_path_buf();
        let mut manager = Self::with_models_provider_and_home(auth, provider, opencli_home);
        manager._test_opencli_home_guard = Some(temp_dir);
        manager
    }

    #[cfg(any(test, feature = "test-support"))]
    /// Construct with a dummy AuthManager containing the provided OpenCLIAuth and opencli home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> OpenCLIResult<Arc<OpenCLIThread>> {
        self.state.get_thread(thread_id).await
    }

    pub async fn start_thread(&self, config: Config) -> OpenCLIResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<opencli_protocol::dynamic_tools::DynamicToolSpec>,
    ) -> OpenCLIResult<NewThread> {
        self.state
            .spawn_thread(
                config,
                InitialHistory::New,
    ...
    /// as `Arc<OpenCLIThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<OpenCLIThread>> {
        self.state.threads.write().await.remove(thread_id)
    }

    /// Closes all threads open in this ThreadManager
    pub async fn remove_and_close_all_threads(&self) -> OpenCLIResult<()> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        ...
        threads
            .get(&thread_id)
            .cloned()
            .ok_or_else(|| OpenCLIErr::ThreadNotFound(thread_id))
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> OpenCLIResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<OpenCLIThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        .await?;
        self.finalize_thread_spawn(opencli, thread_id).await
    }
    ...
    async fn finalize_thread_spawn(
        ...
        let event = opencli.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(OpenCLIErr::SessionConfiguredNotFirstEvent);
            }
        };

        let thread = Arc::new(OpenCLIThread::new(
            opencli,

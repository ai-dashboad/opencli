...
use opencli_protocol::protocol::ApplyPatchApprovalRequestEvent;
use opencli_protocol::protocol::Event;
use opencli_protocol::protocol::EventMsg;
use opencli_protocol::protocol::ExecApprovalRequestEvent;
use opencli_protocol::protocol::Op;
use opencli_protocol::protocol::RequestUserInputEvent;
use opencli_protocol::protocol::SessionSource;
use opencli_protocol::protocol::SubAgentSource;
use opencli_protocol::protocol::Submission;
...
pub(crate) async fn run_opencli_thread_interactive(
    config: Config,
...
) -> Result<OpenCLI, OpenCLIErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);

    let OpenCLISpawnOk { opencli, .. } = OpenCLI::spawn(
        config,
        auth_manager,
        models_manager,
    ...
    let opencli_for_events = Arc::clone(&opencli);
    ...
    let opencli_for_ops = Arc::clone(&opencli);
    tokio::spawn(async move {
        forward_ops(opencli_for_ops, rx_ops, cancel_token_ops).await;
    });
    ...
    Ok(OpenCLI {
        ...
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: opencli.agent_status.clone(),
        session: Arc::clone(&opencli.session),
    })
}
...
pub(crate) async fn run_opencli_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: Arc<ModelsManager>,
    input: Vec<UserInput>,
    ...
    let io = run_opencli_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&opencli).await;
                break;
            }
            event = opencli.next_event() => {
...
async fn shutdown_delegate(opencli: &OpenCLI) {
    let _ = opencli.submit(Op::Interrupt).await;
    let _ = opencli.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = opencli.next_event().await {
            if matches!(
                event.msg,
...
async fn handle_exec_approval(
    ...
    let _ = opencli.submit(Op::ExecApproval { id, decision }).await;
}
...
async fn handle_patch_approval(
    ...
    .await;
    let _ = opencli.submit(Op::PatchApproval { id, decision }).await;
}
...
async fn handle_request_user_input(
    ...
    .await;
    let _ = opencli.submit(Op::UserInputAnswer { id, response }).await;
}
...
async fn await_approval_with_cancel<F>(
    ...
    sub_id: &str,
    cancel_token: &CancellationToken,
) -> opencli_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = opencli_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(sub_id, opencli_protocol::protocol::ReviewDecision::Abort)
                .await;
            opencli_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
mod tests {
    ...
    async fn forward_events_cancelled_while_send_blocked_shuts_down_delegate() {
        ...
        let (session, ctx, _rx_evt) = crate::opencli::make_session_and_context_with_rx().await;
        let opencli = Arc::new(OpenCLI {
            next_id: AtomicU64::new(0),
            tx_sub,
            rx_event: rx_events,

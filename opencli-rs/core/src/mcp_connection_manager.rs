...
//!
//! The [`McpConnectionManager`] owns one [`opencli_rmcp_client::RmcpClient`] per
...
pub const MCP_SANDBOX_STATE_CAPABILITY: &str = "opencli/sandbox-state";

/// Custom MCP request to push sandbox state updates.
/// When used, the `params` field of the notification is [`SandboxState`].
pub const MCP_SANDBOX_STATE_METHOD: &str = "opencli/sandbox-state/update";
...
impl McpConnectionManager {
    ...
    pub async fn list_all_tools(&self) -> HashMap<String, ToolInfo> {
        ...
        for (server_name, managed_client) in &self.clients {
            let client = if server_name == OPENCLI_APPS_MCP_SERVER_NAME {
                // Avoid blocking on opencli_apps_mcp startup; use tools only when ready.
...
fn normalize_opencli_apps_tool_title(
    server_name: &str,
    connector_name: Option<&str>,
    value: &str,
) -> String {
    if server_name != OPENCLI_APPS_MCP_SERVER_NAME {
...
async fn start_server_task(
    ...
    let params = mcp_types::InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "opencli-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("OpenCLI".into()),
            // This field is used by OpenCLI when it is an MCP
            // server: it should not be used when OpenCLI is
            // an MCP client.
            user_agent: None,
        },
...
fn mcp_init_error_display(
    ...
    {
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = OPENCLI_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
    } else if is_mcp_client_auth_required_error(err) {
        format!(
            "The {server_name} MCP server is not logged in. Run `opencli mcp login {server_name}`."
        )

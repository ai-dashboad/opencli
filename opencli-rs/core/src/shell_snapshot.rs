...
impl ShellSnapshot {
    pub fn start_snapshotting(
        ...
        tokio::spawn(
            async move {
                let timer = otel_manager.start_timer("opencli.shell_snapshot.duration_ms", &[]);
                let snapshot =
                    ShellSnapshot::try_new(&opencli_home, snapshot_session_id, &snapshot_shell)
                        .await
                        .map(Arc::new);
                let success = if snapshot.is_some() { "true" } else { "false" };
                let _ = timer.map(|timer| timer.record(&[("success", success)]));
                otel_manager.counter("opencli.shell_snapshot", 1, &[("success", success)]);
    ...
    async fn try_new(opencli_home: &Path, session_id: ThreadId, shell: &Shell) -> Option<Self> {
        // File to store the snapshot
        let extension = match shell.shell_type {
            ShellType::PowerShell => "ps1",
            _ => "sh",
        };
        let path = opencli_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.{extension}"));

        // Clean the (unlikely) leaked snapshot files.
        let opencli_home = opencli_home.to_path_buf();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&opencli_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            opencli_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(opencli_home: &Path, active_session_id: ThreadId) -> Result<()> {
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(opencli_home, session_id).await?;
...
mod tests {
    ...
    fn bash_snapshot_filters_invalid_exports() -> Result<()> {
        ...
        assert!(!stdout.contains("NEXTEST_BIN_EXE_opencli-write-config-schema"));
    ...
    async fn write_rollout_stub(opencli_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
        let dir = opencli_home
            .join("sessions")
            .join("2025")
            .join("01")
            .join("01");
    ...
    async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(opencli_home, stale_session).await?;
    ...
    async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(opencli_home, active_session).await?;
        fs::write(&active_snapshot, "active").await?;

        set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

        cleanup_stale_snapshots(opencli_home, active_session).await?;

...
/// is used for every sub-agent spawned by OpenCLI. By doing so, we make sure the guards are
/// scoped to a user session.
#[derive(Clone, Default)]
pub(crate) struct AgentControl {
    /// Weak handle back to the global thread registry/state.
    /// This is `Weak` to avoid reference cycles and shadow persistence of the form
    /// `ThreadManagerState -> OpenCLIThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn spawn_agent(
        ...
        prompt: String,
        session_source: Option<opencli_protocol::protocol::SessionSource>,
    ) -> OpenCLIResult<ThreadId> {
    ...
    pub(crate) async fn send_prompt(
        ...
        let result = state
            ...
            .await;
        if matches!(result, Err(OpenCLIErr::InternalAgentDied)) {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> OpenCLIResult<String> {
        let state = self.upgrade()?;
        state.send_op(agent_id, Op::Interrupt).await
    }

    /// Submit a shutdown request to an existing agent thread.
    pub(crate) async fn shutdown_agent(&self, agent_id: ThreadId) -> OpenCLIResult<String> {
        let state = self.upgrade()?;
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> OpenCLIResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
        let thread = state.get_thread(agent_id).await?;
        Ok(thread.subscribe_status())
    }

    fn upgrade(&self) -> OpenCLIResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| OpenCLIErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
}
...
mod tests {
    ...
    use opencli_protocol::protocol::TurnCompleteEvent;
    ...
    async fn test_config_with_cli_overrides(
        ...
        let config = ConfigBuilder::default()
            .opencli_home(home.path().to_path_buf())
    ...
    impl AgentControlHarness {
        async fn new() -> Self {
            ...
            let manager = ThreadManager::with_models_provider_and_home(
                OpenCLIAuth::from_api_key("dummy"),
        ...
        async fn start_thread(&self) -> (ThreadId, Arc<OpenCLIThread>) {
            let new_thread = self
                .manager
    ...
    async fn send_prompt_errors_when_thread_missing() {
        ...
        assert_matches!(err, OpenCLIErr::ThreadNotFound(id) if id == thread_id);
    }
    ...
    async fn spawn_agent_respects_max_threads_limit() {
        ...
        let OpenCLIErr::AgentLimitReached {
            max_threads: seen_max_threads,
        } = err
        else {
            panic!("expected OpenCLIErr::AgentLimitReached");
        };
    ...
    async fn spawn_agent_limit_shared_across_clones() {
        ...
        let OpenCLIErr::AgentLimitReached { max_threads } = err else {
            panic!("expected OpenCLIErr::AgentLimitReached");
        };

...
use opencli_api::Prompt as ApiPrompt;
use opencli_api::RequestTelemetry;
use opencli_api::ReqwestTransport;
use opencli_api::ResponseAppendWsRequest;
use opencli_api::ResponseCreateWsRequest;
use opencli_api::ResponseStream as ApiResponseStream;
use opencli_api::ResponsesClient as ApiResponsesClient;
use opencli_api::ResponsesOptions as ApiResponsesOptions;
use opencli_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use opencli_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use opencli_api::SseTelemetry;
use opencli_api::TransportError;
use opencli_api::WebsocketTelemetry;
use opencli_api::build_conversation_headers;
use opencli_api::common::Reasoning;
use opencli_api::common::ResponsesWsRequest;
use opencli_api::create_text_param_for_request;
...
use crate::flags::OPENCLI_RS_SSE_FIXTURE;
...
pub const X_OPENCLI_TURN_STATE_HEADER: &str = "x-opencli-turn-state";
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-opencli-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-opencli-turn-state` request header for
...
impl ModelClientSession {
    ...
    pub(crate) fn try_switch_fallback_transport(&mut self) -> bool {
        ...
        if activated {
            ...
            self.state.otel_manager.counter(
                "opencli.transport.fallback_to_http",
                1,
    ...
    async fn websocket_connection(
        &mut self,
        api_provider: opencli_api::Provider,
    ...
    fn responses_request_compression(&self, auth: Option<&crate::auth::OpenCLIAuth>) -> Compression {
        if self
            ...
            && auth.is_some_and(OpenCLIAuth::is_chatgpt_auth)
    ...
    async fn stream_responses_api(&self, prompt: &Prompt) -> Result<ResponseStream> {
        if let Some(path) = &*OPENCLI_RS_SSE_FIXTURE {
            warn!(path, "Streaming from fixture");
            let stream =
                opencli_api::stream_from_fixture(path, self.state.provider.stream_idle_timeout())
                    .map_err(map_api_error)?;
...
/// Adapts the core `Prompt` type into the `opencli-api` payload shape.
...
fn experimental_feature_headers(config: &Config) -> ApiHeaderMap {
    ...
    {
        headers.insert("x-opencli-beta-features", header_value);
    }
    headers
}
...
fn build_responses_headers(
    ...
    {
        headers.insert(X_OPENCLI_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
/// the mapped `OpenCLIErr` is returned to the caller.
...
fn map_unauthorized_status(status: StatusCode) -> OpenCLIErr {

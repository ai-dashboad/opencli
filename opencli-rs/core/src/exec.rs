...
use opencli_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: opencli_protocol::config_types::WindowsSandboxLevel,
    pub justification: Option<String>,
    /// What this command is for, in the model's own words, when it wrote any.
    /// Carried only so the interface can show it; it never affects execution.
...
pub async fn process_exec_tool_call(
    ...
    let ExecParams {
        // Carried for the interface only; execution has no use for it.
        description: _,
        command,
        cwd,
        expiration,
    ...
    let exec_env = manager
        .transform(
            ...
            sandbox_type,
            sandbox_cwd,
            opencli_linux_sandbox_exe.as_ref(),
            windows_sandbox_level,
        )
        .map_err(OpenCLIErr::from)?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    windows_sandbox_level: opencli_protocol::config_types::WindowsSandboxLevel,
    err: &str,
) {
    ...
    let level = if matches!(
        windows_sandbox_level,
        opencli_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = opencli_otel::metrics::global() {
        let _ = metrics.counter(
            "opencli.windows_sandbox.createprocessasuserw_failed",
            1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_opencli_home;
    use opencli_protocol::config_types::WindowsSandboxLevel;
    use opencli_windows_sandbox::run_windows_sandbox_capture;
    use opencli_windows_sandbox::run_windows_sandbox_capture_elevated;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        OpenCLIErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let opencli_home = find_opencli_home().map_err(|err| {
        OpenCLIErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve opencli_home: {err}"
        )))
    })?;
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(OpenCLIErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, OpenCLIErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(OpenCLIErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(OpenCLIErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(OpenCLIErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
                }));
            }
...
pub(crate) mod errors {
    use super::OpenCLIErr;
    use crate::sandboxing::SandboxTransformError;

    impl From<SandboxTransformError> for OpenCLIErr {
        fn from(err: SandboxTransformError) -> Self {
            match err {
                ...
                SandboxTransformError::SeatbeltUnavailable => OpenCLIErr::UnsupportedOperation(
...
mod tests {
    ...
    async fn process_exec_tool_call_respects_cancellation_token() -> Result<()> {
        ...
        let output = match result {
            Err(OpenCLIErr::Sandbox(SandboxErr::Timeout { output })) => output,

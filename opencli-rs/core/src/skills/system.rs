...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".opencli-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: &str = "v1";

/// Returns the on-disk cache location for embedded system skills.
///
/// This is typically located at `OPENCLI_HOME/skills/.system`.
pub(crate) fn system_cache_root_dir(opencli_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(opencli_home)
        .and_then(|opencli_home| system_cache_root_dir_abs(&opencli_home))
...
fn system_cache_root_dir_abs(opencli_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    opencli_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}

/// Installs embedded system skills into `OPENCLI_HOME/skills/.system`.
///
...
/// install is skipped.
pub(crate) fn install_system_skills(opencli_home: &Path) -> Result<(), SystemSkillsError> {
    let opencli_home = AbsolutePathBuf::try_from(opencli_home)
        .map_err(|source| SystemSkillsError::io("normalize opencli home dir", source))?;
    let skills_root_dir = opencli_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&opencli_home)

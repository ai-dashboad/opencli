        ...
        config_layer_stack.get_layers(ConfigLayerStackOrdering::HighestPrecedenceFirst, true)
    {
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // `$OPENCLI_HOME/skills` (user-installed skills).
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                });

                // Embedded system skills are cached under `$OPENCLI_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/opencli/` on Unix, so treat
                // `/etc/opencli/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
...
fn discover_skills_under_root(root: &Path, scope: SkillScope, outcome: &mut SkillLoadOutcome) {
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by OpenCLI itself.
...
mod tests {
    ...
    const REPO_ROOT_CONFIG_DIR_NAME: &str = ".opencli";

    async fn make_config(opencli_home: &TempDir) -> Config {
        make_config_for_cwd(opencli_home, opencli_home.path().to_path_buf()).await
    }

    async fn make_config_for_cwd(opencli_home: &TempDir, cwd: PathBuf) -> Config {
        let trust_root = cwd
            .ancestors()
    ...
    -> anyhow::Result<()> {
        ...
        let system_folder = tmp.path().join("etc/opencli");
        let user_folder = tmp.path().join("home/opencli");
    ...
    fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
        ...
        let dot_opencli = project_root.join(".opencli");
        fs::create_dir_all(&dot_opencli)?;

        let user_file = AbsolutePathBuf::from_absolute_path(user_folder.join("config.toml"))?;
        let project_dot_opencli = AbsolutePathBuf::from_absolute_path(&dot_opencli)?;
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_opencli_folder: project_dot_opencli,
                },
                TomlValue::Table(toml::map::Map::new()),
                "marked untrusted",
            ),
        ...
        assert_eq!(
            got,
            vec![
                (SkillScope::Repo, dot_opencli.join("skills")),
                (SkillScope::User, user_folder.join("skills")),
                (
                    SkillScope::System,
                    user_folder.join("skills").join(".system")
    ...
    fn write_skill(opencli_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
        write_skill_at(&opencli_home.path().join("skills"), dir, name, description)
    }
    ...
    fn write_system_skill(
        ...
        write_skill_at(
            &opencli_home.path().join("skills/.system"),
            dir,
            name,
            description,
        )
    ...
    async fn loads_skill_dependencies_metadata_from_yaml() {
        ...
        let skill_path = write_skill(&opencli_home, "demo", "dep-skill", "from json");
...
"#,
    ...
    async fn loads_skill_interface_metadata_from_yaml() {
        let opencli_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&opencli_home, "demo", "ui-skill", "from json");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn loads_skills_via_symlinked_subdir_for_user_scope() {
        ...
        fs::create_dir_all(opencli_home.path().join("skills")).unwrap();
        symlink_dir(shared.path(), &opencli_home.path().join("skills/shared"));
    ...
    async fn does_not_loop_on_symlink_cycle_for_user_scope() {
        ...
        // Create a cycle:
        //   $OPENCLI_HOME/skills/cycle/loop -> $OPENCLI_HOME/skills/cycle
        let cycle_dir = opencli_home.path().join("skills/cycle");
    ...
    async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
        ...
        let cfg = make_config_for_cwd(&opencli_home, repo_dir.path().to_path_buf()).await;
        let outcome = load_skills(&cfg);
    ...
    async fn system_scope_ignores_symlinked_subdir() {
        ...
        let system_root = opencli_home.path().join("skills/.system");
    ...
    async fn skips_hidden_and_invalid() {
        let opencli_home = tempfile::tempdir().expect("tempdir");
        let hidden_dir = opencli_home.path().join("skills/.hidden");
        ...
        let invalid_dir = opencli_home.path().join("skills/invalid");
    ...
    async fn enforces_length_limits() {
        ...
        write_skill(&opencli_home, "max-len", "max-len", &max_desc);
        let cfg = make_config(&opencli_home).await;
        ...
        write_skill(&opencli_home, "too-long", "too-long", &too_long_desc);
        let outcome = load_skills(&cfg);
        assert_eq!(outcome.skills.len(), 1);
        assert_eq!(outcome.errors.len(), 1);
        assert!(
    ...
    #[tokio::test]
    async fn loads_skills_from_agents_dir_without_opencli_dir() {
        let opencli_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
    ...
    #[tokio::test]
    async fn loads_skills_from_all_opencli_dirs_under_project_root() {
        let opencli_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
        ...
        let cfg = make_config_for_cwd(&opencli_home, nested_dir).await;
    ...
    #[tokio::test]
    async fn loads_skills_from_opencli_dir_when_not_git_repo() {
        let opencli_home = tempfile::tempdir().expect("tempdir");
        ...
        let cfg = make_config_for_cwd(&opencli_home, work_dir.path().to_path_buf()).await;
    ...
    async fn keeps_duplicate_names_from_repo_and_user() {
        ...
        let user_skill_path = write_skill(&opencli_home, "user", "dupe-skill", "from user");
    ...
    #[tokio::test]
    async fn keeps_duplicate_names_from_nested_opencli_dirs() {
        let opencli_home = tempfile::tempdir().expect("tempdir");
        let repo_dir = tempfile::tempdir().expect("tempdir");
        mark_as_git_repo(repo_dir.path());
    ...
    async fn repo_skills_search_does_not_escape_repo_root() {
        ...
        let cfg = make_config_for_cwd(&opencli_home, repo_dir).await;
    ...
    async fn loads_skills_when_cwd_is_file_in_repo() {
        ...
        let cfg = make_config_for_cwd(&opencli_home, file_path).await;
    ...
    async fn loads_skills_from_system_cache_when_present() {
        ...
        let skill_path = write_system_skill(&opencli_home, "system", "system-skill", "from system");

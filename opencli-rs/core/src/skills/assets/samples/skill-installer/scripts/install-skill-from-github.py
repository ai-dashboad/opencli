#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $OPENCLI_HOME/skills."""
...
def _opencli_home() -> str:
    return os.environ.get("OPENCLI_HOME", os.path.expanduser("~/.opencli"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "opencli")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "opencli-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_opencli_home(), "skills")

...
use opencli_execpolicy::AmendError;
use opencli_execpolicy::Decision;
use opencli_execpolicy::Error as ExecPolicyRuleError;
use opencli_execpolicy::Evaluation;
use opencli_execpolicy::Policy;
use opencli_execpolicy::PolicyParser;
use opencli_execpolicy::RuleMatch;
use opencli_execpolicy::blocking_append_allow_prefix_rule;
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: opencli_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(opencli_home);
...
pub async fn load_exec_policy(config_stack: &ConfigLayerStack) -> Result<Policy, ExecPolicyError> {
    ...
    // Include disabled project layers so .opencli/rules still applies when
...
fn default_policy_path(opencli_home: &Path) -> PathBuf {
    opencli_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
/// - Note: we only use this amendment when the command fails to run in sandbox and opencli prompts the user to run outside the sandbox
...
mod tests {
    ...
    fn config_stack_for_dot_opencli_folder(dot_opencli_folder: &Path) -> ConfigLayerStack {
    ...
    async fn returns_empty_policy_when_feature_disabled() {
        ...
        let config_stack = config_stack_for_dot_opencli_folder(temp_dir.path());
    ...
    async fn loads_rules_from_disabled_project_layers() -> anyhow::Result<()> {
        ...
        let project_dot_opencli_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
        let layers = vec![ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_opencli_folder: project_dot_opencli_folder,
            },
            TomlValue::Table(Default::default()),
            "trust disabled",
        )];
...
"#;
    ...
    async fn append_execpolicy_amendment_updates_policy_and_file() {
        ...
        manager
            .append_amendment_and_update(opencli_home.path(), &ExecPolicyAmendment::from(prefix))
            .await
        ...
        let contents = fs::read_to_string(default_policy_path(opencli_home.path()))
...
"#
    ...
    async fn append_execpolicy_amendment_rejects_empty_prefix() {
        ...
        let result = manager
            .append_amendment_and_update(opencli_home.path(), &ExecPolicyAmendment::from(vec![]))
            .await;

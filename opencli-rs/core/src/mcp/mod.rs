...
pub(crate) const OPENCLI_APPS_MCP_SERVER_NAME: &str = "opencli_apps";
const OPENCLI_CONNECTORS_TOKEN_ENV_VAR: &str = "OPENCLI_CONNECTORS_TOKEN";

fn opencli_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(OPENCLI_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(OPENCLI_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(OPENCLI_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn opencli_apps_mcp_bearer_token(auth: Option<&OpenCLIAuth>) -> Option<String> {
...
fn opencli_apps_mcp_http_headers(auth: Option<&OpenCLIAuth>) -> Option<HashMap<String, String>> {
    let mut headers = HashMap::new();
    if let Some(token) = opencli_apps_mcp_bearer_token(auth) {
        headers.insert("Authorization".to_string(), format!("Bearer {token}"));
    }
    if let Some(account_id) = auth.and_then(OpenCLIAuth::get_account_id) {
...
fn opencli_apps_mcp_url(base_url: &str) -> String {
    ...
    } else if base_url.contains("/api/opencli") {
        format!("{base_url}/apps")
    } else {
        format!("{base_url}/api/opencli/apps")
    }
}

fn opencli_apps_mcp_server_config(config: &Config, auth: Option<&OpenCLIAuth>) -> McpServerConfig {
    let bearer_token_env_var = opencli_apps_mcp_bearer_token_env_var();
    let http_headers = if bearer_token_env_var.is_some() {
        None
    } else {
        opencli_apps_mcp_http_headers(auth)
    };
    let url = opencli_apps_mcp_url(&config.chatgpt_base_url);
...
pub(crate) fn with_opencli_apps_mcp(
    ...
    if connectors_enabled {
        servers.insert(
            OPENCLI_APPS_MCP_SERVER_NAME.to_string(),
            opencli_apps_mcp_server_config(config, auth),
        );
    } else {
        servers.remove(OPENCLI_APPS_MCP_SERVER_NAME);
    }
    servers
}

...
pub type Result<T> = std::result::Result<T, OpenCLIErr>;
...
pub enum OpenCLIErr {
    ...
    /// could not be captured). Analogous to the previous `OpenCLIError::Spawn` variant.
    ...
    #[error(
        "To use OpenCLI with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("opencli-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for OpenCLIErr {
    fn from(_: CancelErr) -> Self {
        OpenCLIErr::TurnAborted
    }
}
...
impl OpenCLIErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            OpenCLIErr::TurnAborted
            | OpenCLIErr::Interrupted
            | OpenCLIErr::EnvVar(_)
            | OpenCLIErr::Fatal(_)
            | OpenCLIErr::UsageNotIncluded
            | OpenCLIErr::QuotaExceeded
            | OpenCLIErr::InvalidImageRequest()
            | OpenCLIErr::InvalidRequest(_)
            | OpenCLIErr::RefreshTokenFailed(_)
            | OpenCLIErr::UnsupportedOperation(_)
            ...
            | OpenCLIErr::InternalAgentDied
            | OpenCLIErr::Io(_)
            | OpenCLIErr::Json(_)
            | OpenCLIErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            OpenCLIErr::LandlockRuleset(_) | OpenCLIErr::LandlockPathFd(_) => false,
        }
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/opencli/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using OpenCLI (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/opencli/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
impl OpenCLIErr {
    /// Minimal shim so that existing `e.downcast_ref::<OpenCLIErr>()` checks continue to compile
    ...
    pub fn to_opencli_protocol_error(&self) -> OpenCLIErrorInfo {
        match self {
            ...
            OpenCLIErr::ModelCap(err) => OpenCLIErrorInfo::ModelCap {
                model: err.model.clone(),
                reset_after_seconds: err.reset_after_seconds,
            },
            OpenCLIErr::RetryLimit(_) => OpenCLIErrorInfo::ResponseTooManyFailedAttempts {
            ...
            OpenCLIErr::SessionConfiguredNotFirstEvent
            | OpenCLIErr::InternalServerError
            | OpenCLIErr::InternalAgentDied => OpenCLIErrorInfo::InternalServerError,
            OpenCLIErr::UnsupportedOperation(_)
            | OpenCLIErr::ThreadNotFound(_)
            | OpenCLIErr::AgentLimitReached { .. } => OpenCLIErrorInfo::BadRequest,
            OpenCLIErr::Sandbox(_) => OpenCLIErrorInfo::SandboxError,
            _ => OpenCLIErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            opencli_error_info: Some(self.to_opencli_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            OpenCLIErr::RetryLimit(err) => Some(err.status),
            OpenCLIErr::UnexpectedStatus(err) => Some(err.status),
            OpenCLIErr::ConnectionFailed(err) => err.source.status(),
            OpenCLIErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
pub fn get_error_message_ui(e: &OpenCLIErr) -> String {
    let message = match e {
        OpenCLIErr::Sandbox(SandboxErr::Denied { output }) => {
        ...
        OpenCLIErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
...
mod tests {
    ...
    fn usage_limit_reached_error_formats_plus_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/opencli/settings/usage to purchase more credits or try again later."
        );
    }
    ...
    fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
        ...
        let err = OpenCLIErr::Sandbox(SandboxErr::Denied {
    ...
    fn to_error_event_handles_response_stream_failed() {
        ...
        let err = OpenCLIErr::ResponseStreamFailed(ResponseStreamFailed {
            source,
            request_id: Some("req-123".to_string()),
        });
        ...
        assert_eq!(
            event.opencli_error_info,
            Some(OpenCLIErrorInfo::ResponseStreamConnectionFailed {
    ...
    fn usage_limit_reached_error_formats_free_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Plus to continue using OpenCLI (https://chatgpt.com/explore/plus), or try again later."
        );
    }
    ...
    fn usage_limit_reached_error_formats_pro_plan_with_reset() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Visit https://chatgpt.com/opencli/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn usage_limit_reached_includes_hours_and_minutes() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/opencli/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn usage_limit_reached_with_promo_message() {
        ...
        with_now_override(base, move || {
            ...
            let err = UsageLimitReachedError {
                ...
                rate_limits: Some(rate_limit_snapshot()),
                promo_message: Some(
                    "To continue using OpenCLI, start a free trial of <PLAN> today".to_string(),
                ),
            };
            let expected = format!(
                "You've hit your usage limit. To continue using OpenCLI, start a free trial of <PLAN> today, or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }

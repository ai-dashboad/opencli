...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let rollout_path = test.opencli.rollout_path().expect("rollout path");
    let db_path = test.config.opencli_home.join(STATE_DB_FILENAME);
    ...
    let db = test.opencli.state_db().expect("state db enabled");
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_opencli()
        .with_pre_build_hook(move |opencli_home| {
            let rollout_path = opencli_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = test.config.opencli_home.join(STATE_DB_FILENAME);
    let rollout_path = test.config.opencli_home.join(&rollout_rel_path);
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let subscriber = tracing_subscriber::registry().with(opencli_state::log_db::start(db.clone()));
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = opencli_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };

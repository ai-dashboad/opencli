...
use opencli_core::exec::ExecToolCallOutput;
use opencli_core::exec::SandboxType;
...
use opencli_core::spawn::OPENCLI_SANDBOX_ENV_VAR;
use opencli_protocol::config_types::WindowsSandboxLevel;
use tempfile::TempDir;

use opencli_core::error::Result;

use opencli_core::get_platform_sandbox;

fn skip_test() -> bool {
    if std::env::var(OPENCLI_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{OPENCLI_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }

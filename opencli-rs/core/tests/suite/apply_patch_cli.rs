...
use core_test_support::test_opencli::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestOpenCLIHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestOpenCLIBuilder) -> TestOpenCLIBuilder,
) -> Result<TestOpenCLIHarness> {
    let builder = configure(test_opencli()).with_config(|config| {

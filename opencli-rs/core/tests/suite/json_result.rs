...
async fn opencli_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    opencli_returns_json_result("gpt-5.1".to_string()).await
}
...
async fn opencli_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("opencli_output_schema".into()))
    ...
    let TestOpenCLI { opencli, cwd, .. } = test_opencli().build(&server).await?;
    ...
    let message = wait_for_event(&opencli, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;

...
async fn undo_harness() -> Result<TestOpenCLIHarness> {
    let builder = test_opencli().with_model("gpt-5.1").with_config(|config| {
        config.include_apply_patch_tool = true;
        config.features.enable(Feature::GhostCommit);
    });
    TestOpenCLIHarness::with_builder(builder).await
}
...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "OpenCLI Tests"])?;
    git(path, &["config", "user.email", "opencli-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by OpenCLI.\n")?;
...
async fn invoke_undo(opencli: &Arc<OpenCLIThread>) -> Result<UndoCompletedEvent> {
    opencli.submit(Op::Undo).await?;
    let event = wait_for_event_match(opencli, |msg| match msg {
...
async fn expect_successful_undo(opencli: &Arc<OpenCLIThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(opencli).await?;
    assert!(
        event.success,
...
async fn expect_failed_undo(opencli: &Arc<OpenCLIThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(opencli).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let opencli = Arc::clone(&harness.test().opencli);
    let completed = expect_successful_undo(&opencli).await?;
...
async fn undo_sequential_turns_consumes_snapshots() -> Result<()> {
    ...
    let opencli = Arc::clone(&harness.test().opencli);
    expect_successful_undo(&opencli).await?;
    assert_eq!(fs::read_to_string(&story)?, "turn two\n");
    ...
    expect_failed_undo(&opencli).await?;

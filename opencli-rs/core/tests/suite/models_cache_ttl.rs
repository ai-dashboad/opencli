...
const REMOTE_MODEL: &str = "opencli-test-ttl";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.opencli_home.join(CACHE_FILE);
    ...
    let _ = wait_for_event(&opencli, |event| matches!(event, EventMsg::TurnComplete(_))).await;

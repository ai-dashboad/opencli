...
async fn opencli_delegate_forwards_exec_approval_and_proceeds_on_approval() {
    skip_if_no_network!();
    ...
    let mut builder = test_opencli().with_model("gpt-5.1").with_config(|config| {
        config.approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
        config.sandbox_policy = Constrained::allow_any(SandboxPolicy::ReadOnly);
    });
    let test = builder.build(&server).await.expect("build test opencli");
    ...
    wait_for_event(&test.opencli, |ev| {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn opencli_delegate_forwards_patch_approval_and_proceeds_on_decision() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn opencli_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.opencli, |_| true).await;
        match ev {

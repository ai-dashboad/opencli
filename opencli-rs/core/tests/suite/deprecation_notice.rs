...
use opencli_core::config_loader::ConfigRequirements;
...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    let TestOpenCLI { opencli, .. } = builder.build(&server).await?;

    let notice = wait_for_event_match(&opencli, |event| match event {
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable unified_exec` or `[features].unified_exec` in config.toml. See https://github.com/ai-dashboad/opencli/blob/main/docs/config.md#feature-flags for details."
        ),
    );

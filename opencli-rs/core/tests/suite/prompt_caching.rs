...
use opencli_core::shell::Shell;
use opencli_core::shell::default_user_shell;
...
async fn prompt_tools_are_consistent_across_requests() -> anyhow::Result<()> {
    ...
    let TestOpenCLI {
        ...
        .with_config(|config| {
            ...
            config.model = Some("gpt-5.1-opencli-max".to_string());
...
async fn opencli_mini_latest_tools() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    use pretty_assertions::assert_eq;
    ...
    let TestOpenCLI { opencli, .. } = test_opencli()
        .with_config(|config| {
            ...
            config.features.enable(Feature::CollaborationModes);
            config.model = Some("opencli-mini-latest".to_string());
        })
        .build(&server)
        .await?;

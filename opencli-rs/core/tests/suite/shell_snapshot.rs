...
async fn wait_for_snapshot(opencli_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = opencli_home.join("shell_snapshots");
...
async fn run_snapshot_command(command: &str) -> Result<SnapshotRun> {
    let builder = test_opencli().with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config.features.enable(Feature::UnifiedExec);
        config.features.enable(Feature::ShellSnapshot);
    });
    let harness = TestOpenCLIHarness::with_builder(builder).await?;
    let args = json!({
        "cmd": command,
        "yield_time_ms": 1000,
    });
    ...
    let opencli_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&opencli_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&opencli, |ev| match ev {
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.opencli_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let opencli_home = home.path().to_path_buf();
    let opencli = harness.test().opencli.clone();

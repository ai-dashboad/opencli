...
async fn request_body_is_zstd_compressed_for_opencli_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/opencli/v1", server.uri());
    ...
    let opencli = builder.build(&server).await?.opencli;

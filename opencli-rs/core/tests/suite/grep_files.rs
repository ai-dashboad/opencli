...
async fn grep_files_tool_collects_matches() -> Result<()> {
    ...
    let test = build_test_opencli(&server).await?;
...
async fn build_test_opencli(server: &wiremock::MockServer) -> Result<TestOpenCLI> {
    let mut builder = test_opencli().with_model(MODEL_WITH_TOOL);

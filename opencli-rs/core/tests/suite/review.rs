...
use opencli_core::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use opencli_core::protocol::EventMsg;
use opencli_core::protocol::ExitedReviewModeEvent;
use opencli_core::protocol::Op;
use opencli_core::protocol::ReviewCodeLocation;
use opencli_core::protocol::ReviewFinding;
use opencli_core::protocol::ReviewLineRange;
use opencli_core::protocol::ReviewOutputEvent;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under OpenCLI sandbox network restrictions.
    skip_if_no_network!();
    ...
    let opencli = new_conversation_for_server(&server, opencli_home.clone(), |_| {}).await;
    ...
    let path = opencli.rollout_path().expect("rollout path");
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&opencli, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let opencli = new_conversation_for_server(&server, opencli_home.clone(), |cfg| {
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = opencli_home.path().join("resume.jsonl");
    {
        ...
        let user = opencli_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = opencli_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let opencli =
        resume_conversation_for_server(&server, opencli_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let opencli = new_conversation_for_server(&server, opencli_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, opencli_home, resume_path)
        .await

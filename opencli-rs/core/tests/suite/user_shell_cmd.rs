...
use opencli_core::protocol::ExecOutputStream;
...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    opencli
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&opencli, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _ = wait_for_event(&opencli, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;
    opencli.submit(Op::Interrupt).await.unwrap();

    // Expect a TurnAborted(Interrupted) notification.
    let msg = wait_for_event(&opencli, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_opencli::test_opencli().with_config(move |config| {
        config.features.disable(Feature::ShellSnapshot);
    });
    let test = builder.build(&server).await?;

    #[cfg(windows)]
    let command = r#"$val = $env:OPENCLI_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${OPENCLI_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.opencli, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.opencli, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.opencli, |ev| match ev {
    ...
    let _ = wait_for_event(&test.opencli, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_opencli::test_opencli();
    let test = builder
        .with_config(|config| {

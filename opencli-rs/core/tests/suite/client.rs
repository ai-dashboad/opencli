...
use opencli_core::LocalShellStatus;
...
use opencli_core::TransportManager;
use opencli_core::WireApi;
use opencli_core::auth::AuthCredentialsStoreMode;
use opencli_core::built_in_model_providers;
use opencli_core::default_client::originator;
...
/// Writes an `auth.json` into the provided `opencli_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        opencli_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = opencli_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![opencli_protocol::models::ContentItem::InputText {
    ...
    let prior_system = opencli_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![opencli_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = opencli_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure OpenCLI to resume from our file
    let opencli_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_opencli()
        .with_home(opencli_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, opencli_home, session_path.clone())
        .await
        .expect("resume conversation");
    let opencli = test.opencli.clone();
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    let mut builder = test_opencli().with_auth(OpenCLIAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_opencli()
        .with_auth(OpenCLIAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    model_provider.base_url = Some(format!("{}/api/opencli", server.uri()));
    let mut builder = test_opencli()
        .with_auth(create_dummy_opencli_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/opencli/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    // Init session
    let opencli_home = TempDir::new().unwrap();
    ...
    let auth_manager =
        match OpenCLIAuth::from_auth_storage(opencli_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => opencli_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No OpenCLIAuth found in opencli_home"),
            Err(e) => panic!("Failed to load OpenCLIAuth: {e}"),
        };
...
async fn skills_append_to_instructions() {
    ...
    let opencli_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = opencli_home.path().join("skills/demo");
    ...
    let opencli_home_path = opencli_home.path().to_path_buf();
    let mut builder = test_opencli()
        .with_home(opencli_home.clone())
        .with_auth(OpenCLIAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = opencli_home_path;
        });
    let opencli = builder
        .build(&server)
        .await
    ...
    let _opencli_home_guard = opencli_home;
}
...
-> anyhow::Result<()> {
    ...
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    let TestOpenCLI { opencli, .. } = test_opencli().with_model("gpt-5.1").build(&server).await?;
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-opencli-primary-used-percent", "12.5")
        .insert_header("x-opencli-secondary-used-percent", "40.0")
        .insert_header("x-opencli-primary-window-minutes", "10")
        .insert_header("x-opencli-secondary-window-minutes", "60")
        .insert_header("x-opencli-primary-reset-at", "1704069000")
        .insert_header("x-opencli-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_opencli()
        .with_auth(OpenCLIAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    let first_token_event =
        wait_for_event(&opencli, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    wait_for_event(&opencli, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-opencli-primary-used-percent", "100.0")
        .insert_header("x-opencli-secondary-used-percent", "87.5")
        .insert_header("x-opencli-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-opencli-primary-window-minutes", "15")
        .insert_header("x-opencli-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_opencli();
    let opencli_fixture = builder.build(&server).await?;
    let opencli = opencli_fixture.opencli.clone();
    ...
    let token_event = wait_for_event(&opencli, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&opencli, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&opencli, |event| {
        matches!(
            event,
...
fn create_dummy_opencli_auth() -> OpenCLIAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under OpenCLI sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();

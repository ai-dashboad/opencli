...
async fn responses_api_emits_api_request_event() {
    ...
    let TestOpenCLI { opencli, .. } = test_opencli().build(&server).await.unwrap();
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("opencli.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected opencli.api_request event".to_string()))
    });

    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("opencli.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected opencli.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("opencli.sse_event")
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing opencli.sse_event".to_string()))
    });
}
...
async fn process_sse_failed_event_logs_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("opencli.sse_event") && line.contains("event.kind=response.failed")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("missing opencli.sse_event".to_string()))
    });
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    wait_for_event(&opencli, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("opencli.tool_result") && line.contains("call_id=custom-tool-call")
            })
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("opencli.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing opencli.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_local_shell_missing_ids() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("opencli.tool_result")

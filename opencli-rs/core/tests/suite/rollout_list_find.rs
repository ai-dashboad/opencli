...
use opencli_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(opencli_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = opencli_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(opencli_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(opencli_home, "sessions", id)
}
...
#[tokio::test]
async fn find_handles_gitignore_covering_opencli_home_directory() {
    let repo = TempDir::new().unwrap();
    let opencli_home = repo.path().join(".opencli");
    std::fs::create_dir_all(&opencli_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".opencli/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&opencli_home, id);

    let found = find_thread_path_by_id_str(&opencli_home, &id.to_string())
        .await
        .unwrap();

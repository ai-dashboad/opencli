...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestOpenCLI) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestOpenCLI, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.opencli, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestOpenCLI) {
    let event = wait_for_event(&test.opencli, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestOpenCLI) {
    wait_for_event(&test.opencli, |event| {
...
fn scenarios() -> Vec<ScenarioSpec> {
    ...
    vec![
        ...
        ScenarioSpec {
            ...
            sandbox_permissions: SandboxPermissions::UseDefault,
            features: vec![],
            model_override: Some("gpt-5.1-opencli"),
            outcome: Outcome::Auto,
            expectation: Expectation::PatchApplied {
                target: TargetPath::Workspace("apply_patch_function.txt"),
                content: "function-apply-patch",
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let mut builder = test_opencli().with_model(model).with_config(move |config| {

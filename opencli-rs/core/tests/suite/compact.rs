...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn OpenCLI.
    ...
    let warning_event = wait_for_event(&opencli, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down OpenCLI to flush rollout entries before inspecting the file.
    opencli.submit(Op::Shutdown).await.unwrap();
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let opencli = builder.build(&server).await.unwrap().opencli;
    ...
    let first = wait_for_event_match(&opencli, |ev| match ev {
    ...
    let last = wait_for_event_match(&opencli, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    {
        let event = opencli.next_event().await.unwrap();
        match event.msg {
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        opencli_protocol::models::ResponseItem::Message {
            id: None,
        ...
        opencli_protocol::models::ResponseItem::Compaction {
    ...
    let mut resume_builder = test_opencli().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        config.features.enable(Feature::RemoteCompaction);
    });
    ...
    wait_for_event(&resumed.opencli, |event| {
...
async fn auto_compact_counts_encrypted_reasoning_before_last_user() {
    ...
    let opencli = test_opencli()
        .with_auth(OpenCLIAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            set_test_compact_prompt(config);

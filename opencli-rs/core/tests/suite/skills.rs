...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_opencli().with_pre_build_hook(|home| {
        write_skill(home, "demo", "demo skill", skill_body);
    });
    let test = builder.build(&server).await?;

    let skill_path = test.opencli_home_path().join("skills/demo/SKILL.md");
    ...
    core_test_support::wait_for_event(test.opencli.as_ref(), |event| {
        matches!(event, opencli_core::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.opencli_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, opencli_protocol::protocol::SkillScope::System);

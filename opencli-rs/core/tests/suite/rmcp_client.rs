...
use opencli_utils_cargo_bin::cargo_bin;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.opencli, |ev| {
    ...
    let end_event = wait_for_event(&fixture.opencli, |ev| {
...
async fn stdio_image_completions_round_trip() -> anyhow::Result<()> {
    ...
    let fixture = test_opencli()
        .with_config(move |config| {
            config.model_provider.wire_api = opencli_core::WireApi::Chat;
            let mut servers = config.mcp_servers.get().clone();
            servers.insert(
                server_name.to_string(),
                McpServerConfig {
...
/// This test writes to a fallback credentials file in OPENCLI_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire OPENCLI_HOME through the code.
#[serial(opencli_home)]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("OPENCLI_HOME", temp_home.path().as_os_str());

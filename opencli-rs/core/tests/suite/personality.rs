...
use opencli_core::config::types::Personality;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    let opencli_home = TempDir::new().expect("create temp dir");
...
async fn user_turn_personality_none_does_not_add_update_message() -> anyhow::Result<()> {
    ...
    wait_for_event(&test.opencli, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    .await;
    let mut builder = test_opencli()
        .with_model("exp-opencli-personality")
        .with_config(|config| {
            config.features.disable(Feature::RemoteModels);
            config.features.enable(Feature::Personality);
        });
...
async fn ignores_remote_personality_if_remote_models_disabled() -> anyhow::Result<()> {
    ...
    let mut builder = test_opencli()
        .with_auth(opencli_core::OpenCLIAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config.features.disable(Feature::RemoteModels);
            config.features.enable(Feature::Personality);
    ...
    assert!(
        instructions_text.contains("You are OpenCLI, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals."),
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "opencli-remote-default-personality";
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "opencli-remote-personality";
...
async fn wait_for_model_available(
    ...
    slug: &str,
    config: &opencli_core::config::Config,
) {

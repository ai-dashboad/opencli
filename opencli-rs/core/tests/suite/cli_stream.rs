...
fn repo_root() -> std::path::PathBuf {
    ...
    opencli_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
/// 2. Configures opencli to use this mock server via a custom provider
...
async fn chat_mode_stream_cli() {
    ...
    let bin = opencli_utils_cargo_bin::cargo_bin("opencli").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("OPENCLI_HOME", home.path())
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // OpenCLI to use the Responses API with the dummy env var.
    let provider_override = format!(
...
/// 2. Configures opencli to read from this fixture via OPENCLI_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("OPENCLI_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("OPENCLI_RS_SSE_FIXTURE", fixture)
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the opencli CLI and invoke `exec`, which is what records a session.
    let bin = opencli_utils_cargo_bin::cargo_bin("opencli").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        ...
        .arg(&prompt);
    cmd.env("OPENCLI_HOME", home.path())
        .env(OPENCLI_API_KEY_ENV_VAR, "dummy")
        .env("OPENCLI_RS_SSE_FIXTURE", &fixture)
    ...
    let bin2 = opencli_utils_cargo_bin::cargo_bin("opencli").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("OPENCLI_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("OPENCLI_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume opencli-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = opencli_core::git_info::collect_git_info(&git_repo).await;

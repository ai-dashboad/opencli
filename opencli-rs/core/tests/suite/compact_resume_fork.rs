...
//! request payload that OpenCLI would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(OPENCLI_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<OpenCLIThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.opencli)
}

async fn user_turn(conversation: &Arc<OpenCLIThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<OpenCLIThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn fetch_conversation_path(conversation: &Arc<OpenCLIThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = opencli_core::AuthManager::from_auth_for_testing(
        opencli_core::OpenCLIAuth::from_api_key("dummy"),
    );
    manager

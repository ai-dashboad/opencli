...
pub mod test_opencli;
pub mod test_opencli_exec;
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\opencli\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.opencli`.
pub async fn load_default_config_for_test(opencli_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        opencli_linux_sandbox_exe: Some(
            opencli_utils_cargo_bin::cargo_bin("opencli-linux-sandbox")
                .expect("should find binary for opencli-linux-sandbox"),
        ),
    ...
    F: FnMut(&opencli_core::protocol::EventMsg) -> bool,
{
    ...
    wait_for_event_with_timeout(opencli, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(opencli: &OpenCLIThread, matcher: F) -> T
where
    F: Fn(&opencli_core::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(opencli, |ev| matcher(ev).is_some()).await;
    matcher(&ev).unwrap()
}
    ...
    F: FnMut(&opencli_core::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(5)), opencli.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    opencli_core::spawn::OPENCLI_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    opencli_core::spawn::OPENCLI_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    opencli_core::shell::default_user_shell().derive_exec_args(command, true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    opencli_core::shell::default_user_shell().derive_exec_args(command, false)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    opencli_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}

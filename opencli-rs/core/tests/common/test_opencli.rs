...
impl TestOpenCLIBuilder {
    ...
    pub fn with_auth(mut self, auth: OpenCLIAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestOpenCLI> {
    ...
    async fn build_from_config(
        ...
        let new_conversation = match resume_from {
            Some(path) => {
                let auth_manager = opencli_core::AuthManager::from_auth_for_testing(auth);
                thread_manager
        ...
        Ok(TestOpenCLI {
            ...
            cwd,
            config,
            opencli: new_conversation.thread,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = opencli_utils_cargo_bin::cargo_bin("opencli") {
            config.opencli_linux_sandbox_exe = Some(path);
        }
...
pub struct TestOpenCLI {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub opencli: Arc<OpenCLIThread>,
...
impl TestOpenCLI {
    ...
    pub fn opencli_home_path(&self) -> &Path {
    ...
    pub async fn submit_turn_with_policies(
        ...
        wait_for_event(&self.opencli, |event| {
...
pub struct TestOpenCLIHarness {
    server: MockServer,
    test: TestOpenCLI,
}

impl TestOpenCLIHarness {
    pub async fn new() -> Result<Self> {
        Self::with_builder(test_opencli()).await
    }

    pub async fn with_config(mutator: impl FnOnce(&mut Config) + Send + 'static) -> Result<Self> {
        Self::with_builder(test_opencli().with_config(mutator)).await
    }

    pub async fn with_builder(mut builder: TestOpenCLIBuilder) -> Result<Self> {
...
pub fn test_opencli() -> TestOpenCLIBuilder {
    TestOpenCLIBuilder {
        config_mutators: vec![],
        auth: OpenCLIAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
        home: None,
    }
}

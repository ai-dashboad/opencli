...
A privileged user (i.e., `root` or a user with `sudo`) who has access to `OPENAI_API_KEY` would run the following to start the server, as `opencli-responses-api-proxy` reads the auth token from `stdin`:

```shell
printenv OPENAI_API_KEY | env -u OPENAI_API_KEY opencli-responses-api-proxy --http-shutdown --server-info /tmp/server-info.json
```

A non-privileged user would then run OpenCLI as follows, specifying the `model_provider` dynamically:

```shell
PROXY_PORT=$(jq .port /tmp/server-info.json)
PROXY_BASE_URL="http://127.0.0.1:${PROXY_PORT}"
opencli exec -c "model_providers.openai-proxy={ name = 'OpenAI Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
...
- Reads the API key from `stdin`. All callers should pipe the key in (for example, `printenv OPENAI_API_KEY | opencli-responses-api-proxy`).
...
```
opencli-responses-api-proxy [--port <PORT>] [--server-info <FILE>] [--http-shutdown] [--upstream-url <URL>]
```
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the OpenCLI CLI expectations.

For Azure, for example (ensure your deployment accepts `Authorization: Bearer <key>`):

```shell
printenv AZURE_OPENAI_API_KEY | env -u AZURE_OPENAI_API_KEY opencli-responses-api-proxy \
  --http-shutdown \
...
- We leverage [`opencli_process_hardening`](https://github.com/ai-dashboad/opencli/blob/main/opencli-rs/process-hardening/README.md) so `opencli-responses-api-proxy` is run with standard process-hardening techniques.

...
use anyhow::Result;
use opencli_core::auth::OpenCLIAuth;
...
impl Client {
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &OpenCLIAuth) -> Result<Self> {
        let token = auth.get_token().map_err(anyhow::Error::from)?;
        let mut client = Self::new(base_url)?
            .with_user_agent(get_opencli_user_agent())
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("opencli-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        let url = match self.path_style {
            PathStyle::OpenCLIApi => format!("{}/api/opencli/usage", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::OpenCLIApi => format!("{}/api/opencli/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::OpenCLIApi => format!("{}/api/opencli/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::OpenCLIApi => format!(
                "{}/api/opencli/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the managed requirements file from opencli-backend.
    ///
    /// `GET /api/opencli/config/requirements` (OpenCLI API style) or
    ...
    pub async fn get_config_requirements_file(&self) -> Result<ConfigFileResponse> {
        let url = match self.path_style {
            PathStyle::OpenCLIApi => format!("{}/api/opencli/config/requirements", self.base_url),
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::OpenCLIApi => format!("{}/api/opencli/tasks", self.base_url),
